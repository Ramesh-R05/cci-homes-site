export default {
    totalCount: 1212,
    data: [
        {
            id: 'HOMES-3240',
            url: '/abhishek-first-gallery-3240',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-3241', 'HOMES-3242', 'HOMES-3240'],
            tags: ['food:Homes navigation:Bedroom', 'food:Topic:Kitchen news'],
            source: 'real living',
            sortOrder: '1',
            contentTags: ['food:Homes navigation:Bedroom', 'food:Topic:Kitchen news'],
            contentYear: 2017,
            tagsDetails: [
                {
                    name: 'food:Topic:Kitchen news',
                    urlName: 'kitchen-news',
                    fullName: 'food_Topic_Kitchen_news',
                    displayName: 'Kitchen news'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                }
            ],
            contentMonth: 2,
            contentTitle: 'This is long title',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'it is a teaser',
            pageSearchTags: ['Bedroom', 'bedroom', 'Kitchen news', 'Kitchen-news', 'kitchen-news', 'kitchen news'],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2017-02-13T02:28:52.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3238',
            url: '/abhisheks-first-article-3238',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-3227', 'HOMES-3239', 'HOMES-3238'],
            tags: ['food:Homes navigation:Kitchen', 'food:Topic:Bathroom news'],
            source: 'Homes To Love',
            sortOrder: '1',
            contentYear: 2017,
            tagsDetails: [
                {
                    name: 'food:Topic:Bathroom news',
                    urlName: 'bathroom-news',
                    fullName: 'food_Topic_Bathroom_news',
                    displayName: 'Bathroom news'
                },
                {
                    name: 'food:Homes navigation:Kitchen',
                    urlName: 'kitchen',
                    fullName: 'food_Homes_navigation_Kitchen',
                    displayName: 'Kitchen'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Excellent outdoor furniture',
            contentVideo: {
                id: '4064490184001',
                name: 'DIY PROJECT: Rope rug - homes+',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4064490184001',
                        statusCode: 200,
                        brightcoveId: '4064490184001',
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4067379056001_RopeMatt-WITHOVERLAY.jpg?pubId=761709621001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4067358475001_RopeMatt-WITHOUT.jpg?pubId=761709621001'
                    }
                }
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Hi this is teaser',
            contentHasVideo: true,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2017-02-12T23:58:02.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageCaption: '<strong>This is the caption of the image and it will be shown for the image</strong>'
        },
        {
            id: 'HOMES-3230',
            url: '/automation-test-article-with-hero-image-3230',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-3227', 'HOMES-3228', 'HOMES-3230'],
            tags: ['food:Building:Type:House', 'food:Homes navigation:Bedroom', 'food:Topic:Social media'],
            source: 'Belle',
            sortOrder: '3',
            contentYear: 2017,
            tagsDetails: [
                {
                    name: 'food:Topic:Social media',
                    urlName: 'social-media',
                    fullName: 'food_Topic_Social_media',
                    displayName: 'Social media'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Building:Type:House',
                    urlName: 'house',
                    fullName: 'food_Building_Type_House',
                    displayName: 'House'
                }
            ],
            contentMonth: 1,
            contentTitle: 'Article Long Title - How to get black and white decorating right Long Title Long Title EOM',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                'Article Short Teaser - Find out how to use a combination of black and white to striking effect in your home. Short Teaser EOM',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2017-01-20T04:27:06.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'Image Alt Text',
            contentImageCaption: 'This is hero image caption Black brings a sense of drama to your home. Photo: Felix Forest'
        },
        {
            id: 'HOMES-3218',
            url: '/abhi-test-3218',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-3209', 'HOMES-3219', 'HOMES-3220', 'HOMES-3218'],
            tags: ['food:Building:Type:House', 'food:Homes navigation:Bedroom', 'food:Topic:Social media'],
            source: 'Belle',
            sortOrder: '1',
            contentTags: [],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Social media',
                    urlName: 'social-media',
                    fullName: 'food_Topic_Social_media',
                    displayName: 'Social media'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Building:Type:House',
                    urlName: 'house',
                    fullName: 'food_Building_Type_House',
                    displayName: 'House'
                }
            ],
            sponsorName: 'Myer Eat Live',
            contentMonth: 12,
            contentTitle: 'Testing to check the editor',
            contentVideo: {
                id: '4067173319001',
                name: 'Cut a mango',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4067173319001',
                        statusCode: 200,
                        brightcoveId: '4067173319001',
                        thumbnailUrl:
                            'http://brightcove.vo.llnwd.net/e1/pd/3639471509001/3639471509001_3895932979001_th-546a7fa5e4b02e255f415f20-1592194027001.jpg?pubId=3639471509001',
                        videoStillUrl:
                            'http://brightcove.vo.llnwd.net/e1/pd/3639471509001/3639471509001_3898256555001_vlcsnap-00016.jpg?pubId=3639471509001'
                    }
                }
            },
            nodeTypeAlias: 'Gallery',
            contentHasVideo: true,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-12-14T10:28:05.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3212',
            url: '/tags-test-3212',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-3212'],
            tags: ['tv:tv_network:TV1', 'food:Cooking time:More than 1 hour', 'food:Topic:In focus', 'food:Homes navigation:Bedroom'],
            source: 'real living',
            sortOrder: '4',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'tv:tv_network:TV1',
                    urlName: 'tv1',
                    fullName: 'tv_tv_network_TV1',
                    displayName: 'TV1'
                },
                {
                    name: 'food:Cooking time:More than 1 hour',
                    urlName: 'more-than-1-hour',
                    fullName: 'food_Cooking_time_More_than_1_hour',
                    displayName: 'More than 1 hour'
                },
                {
                    name: 'food:Topic:In focus',
                    urlName: 'in-focus',
                    fullName: 'food_Topic_In_focus',
                    displayName: 'In focus'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                }
            ],
            contentMonth: 12,
            contentTitle: 'tags test article',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Light and casual or dark and dramatic, this hard-working fabric does double duty in chic home interiors.',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-12-13T04:06:46.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-3211',
            url: '/arvind-test-3211',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-3211'],
            tags: ['food:Topic:In focus', 'food:Homes navigation:Bedroom', 'food:Topic:Decorating 101'],
            source: 'Australian House and Garden',
            sortOrder: '3',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:In focus',
                    urlName: 'in-focus',
                    fullName: 'food_Topic_In_focus',
                    displayName: 'In focus'
                },
                {
                    name: 'food:Topic:Decorating 101',
                    urlName: 'decorating-101',
                    fullName: 'food_Topic_Decorating_101',
                    displayName: 'Decorating 101'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                }
            ],
            contentMonth: 12,
            contentTitle: 'test source',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'test source',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-12-09T00:54:16.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-3210',
            url: '/template-htl-poll-3210',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-3210'],
            tags: ['food:Topic:Luxury home', 'food:Homes navigation:Bedroom'],
            source: 'homes+',
            sortOrder: '2',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home',
                    fullName: 'food_Topic_Luxury_home',
                    displayName: 'Luxury home'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                }
            ],
            contentMonth: 12,
            contentTitle: 'Template HTL Poll',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-12-08T05:04:01.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-2785',
            url: '/halcyon-house-2785',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3207', 'HOMES-2785'],
            tags: ['food:Location and setting:Setting:Beachside', 'food:Topic:Design news', 'food:Homes navigation:DIY'],
            source: 'real living',
            sortOrder: '1',
            contentTags: ['food:Location and setting:Setting:Beachside', 'food:Topic:Design news', 'food:Homes navigation:DIY'],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Design news',
                    urlName: 'design-news',
                    fullName: 'food_Topic_Design_news',
                    displayName: 'Design news'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'food:Location and setting:Setting:Beachside',
                    urlName: 'beachside',
                    fullName: 'food_Location_and_setting_Setting_Beachside',
                    displayName: 'Beachside'
                }
            ],
            contentMonth: 12,
            contentTitle: 'Escape to Halcyon House',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'This luxurious boutique beachside hotel maintains a low key Aussie feel.',
            pageSearchTags: ['Beachside', 'beachside', 'Design news', 'Design-news', 'design-news', 'design news', 'DIY', 'diy'],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/07/2785/HERO_Halycon.jpg',
            pageDateCreated: '2016-12-01T13:00:00.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Halcyon House Hotel ',
            contentImageCaption: 'Halcyon House Hotel Cabarita Beach '
        },
        {
            id: 'HOMES-2828',
            url: '/cool-makeover-for-teen-bedroom-2828',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-2828'],
            tags: [
                "food:Room:Bedroom:Girl's bedroom",
                'food:Room:Bedroom:Teen bedroom',
                'food:Topic:Decorating 101',
                'food:Topic:Before and after',
                'food:Decorating:Style:Contemporary',
                'food:Decorating:Colour:Black and white',
                'food:Homes navigation:Bedroom'
            ],
            source: 'homes+',
            sortOrder: '1',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Decorating 101',
                    urlName: 'decorating-101',
                    fullName: 'food_Topic_Decorating_101',
                    displayName: 'Decorating 101'
                },
                {
                    name: 'food:Topic:Before and after',
                    urlName: 'before-and-after',
                    fullName: 'food_Topic_Before_and_after',
                    displayName: 'Before and after'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Decorating:Colour:Black and white',
                    urlName: 'black-and-white',
                    fullName: 'food_Decorating_Colour_Black_and_white',
                    displayName: 'Black and white'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                },
                {
                    name: 'food:Room:Bedroom:Teen bedroom',
                    urlName: 'teen-bedroom',
                    fullName: 'food_Room_Bedroom_Teen_bedroom',
                    displayName: 'Teen bedroom'
                },
                {
                    name: "food:Room:Bedroom:Girl's bedroom",
                    urlName: 'girls-bedroom',
                    fullName: 'food_Room_Bedroom_Girls_bedroom',
                    displayName: "Girl's bedroom"
                }
            ],
            contentMonth: 12,
            contentTitle: 'Teen bedroom grows up TEST',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: "A cool, modern makeover tranforms 13-year old Emma's bedroom into a stylish retreat she can be proud of.",
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/13/2828/HEROOIMG_297-copy.jpg',
            pageDateCreated: '2016-12-01T13:00:00.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageCaption:
                'Emma chose the decor for her new room, opting for bargains she can mix and match to suit her budget and evolving style. She lives with her mum, dad, sister and Casper, their nine-year-old bichon frise. '
        },
        {
            id: 'HOMES-3205',
            url: '/brand-article-3205',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3189', 'HOMES-3205'],
            tags: ['food:Decorating:Style:Contemporary', 'food:Topic:Profile', 'food:Homes navigation:Living'],
            source: 'Australian House and Garden',
            sortOrder: '5',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Profile',
                    urlName: 'profile',
                    fullName: 'food_Topic_Profile',
                    displayName: 'Profile'
                },
                {
                    name: 'food:Homes navigation:Living',
                    urlName: 'living',
                    fullName: 'food_Homes_navigation_Living',
                    displayName: 'Living'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                }
            ],
            contentMonth: 11,
            contentTitle: 'Brand Artcile',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Brand Artcile test Brand Artcile',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-11-30T01:16:01.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-3203',
            url: '/tag-gallery-3203',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3196', 'HOMES-3203'],
            tags: ['food:Room:Other rooms:Bar room', 'food:Homes navigation:Living'],
            source: 'Belle',
            sortOrder: '4',
            contentTags: ['food:Room:Other rooms:Bar room', 'food:Homes navigation:Living'],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Room:Other rooms:Bar room',
                    urlName: 'bar-room',
                    fullName: 'food_Room_Other_rooms_Bar_room',
                    displayName: 'Bar room'
                },
                {
                    name: 'food:Homes navigation:Living',
                    urlName: 'living',
                    fullName: 'food_Homes_navigation_Living',
                    displayName: 'Living'
                }
            ],
            contentMonth: 11,
            contentTitle: 'Bar Room Tag test',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Bar Room Tag test',
            pageSearchTags: ['Bar room', 'Bar-room', 'bar-room', 'bar room', 'Living', 'living'],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-11-28T00:40:01.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3201',
            url: '/test-stub-gallery-3201',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3196', 'HOMES-3201'],
            tags: [
                'food:Location and setting:Australia:Victoria:Mornington Peninsula',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Coastal/beach',
                'food:Homes navigation:Real Homes'
            ],
            source: 'Australian House and Garden',
            sortOrder: '3',
            contentTags: [
                'food:Location and setting:Australia:Victoria:Mornington Peninsula',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Coastal/beach',
                'food:Homes navigation:Real Homes'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home',
                    fullName: 'food_Topic_Luxury_home',
                    displayName: 'Luxury home'
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                },
                {
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    displayName: 'Coastal/beach'
                },
                {
                    name: 'food:Location and setting:Australia:Victoria:Mornington Peninsula',
                    urlName: 'mornington-peninsula',
                    fullName: 'food_Location_and_setting_Australia_Victoria_Mornington_Peninsula',
                    displayName: 'Mornington Peninsula'
                }
            ],
            contentMonth: 11,
            contentTitle: 'A luxurious bushland retreat',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary:
                'A quiet place on the Mornington Peninsula to relax and recharge has also become a source of inspiration for these creative owners.',
            pageSearchTags: [
                'Mornington Peninsula',
                'Mornington-Peninsula',
                'mornington-peninsula',
                'mornington peninsula',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Coastal/beach',
                'coastal/beach',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-11-23T03:00:27.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Mornington Peninsular home'
        },
        {
            id: 'HOMES-3198',
            url: '/test-preview-3198',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3189', 'HOMES-3198'],
            tags: ['food:Topic:In focus', 'food:Homes navigation:Outdoor'],
            source: 'Australian House and Garden',
            sortOrder: '4',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:In focus',
                    urlName: 'in-focus',
                    fullName: 'food_Topic_In_focus',
                    displayName: 'In focus'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                }
            ],
            contentMonth: 11,
            contentTitle: 'test preview nick',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-11-11T03:35:50.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-3197',
            url: '/new-gallery-3197',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3196', 'HOMES-3197'],
            tags: ['food:Topic:In focus'],
            source: 'Belle',
            sortOrder: '2',
            contentTags: ['food:Topic:In focus'],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:In focus',
                    urlName: 'in-focus',
                    fullName: 'food_Topic_In_focus',
                    displayName: 'In focus'
                }
            ],
            contentMonth: 11,
            contentTitle:
                'new gallery test test test title length test test title length test test title length test test title length test test title length test',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            pageSearchTags: ['In focus', 'In-focus', 'in-focus', 'in focus'],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-11-11T01:17:17.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3195',
            url: '/testgally-3195',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3196', 'HOMES-3195'],
            tags: ['food:Decorating:Style:Country', 'food:Topic:In focus'],
            source: 'Belle',
            sortOrder: '1',
            contentTags: ['food:Decorating:Style:Country', 'food:Topic:In focus'],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:In focus',
                    urlName: 'in-focus',
                    fullName: 'food_Topic_In_focus',
                    displayName: 'In focus'
                },
                {
                    name: 'food:Decorating:Style:Country',
                    urlName: 'country',
                    fullName: 'food_Decorating_Style_Country',
                    displayName: 'Country'
                }
            ],
            contentMonth: 11,
            contentTitle: 'tets Gally TES',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'tets Gally',
            pageSearchTags: ['Country', 'country', 'In focus', 'In-focus', 'in-focus', 'in focus'],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-11-10T06:29:39.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3194',
            url: '/test-article-with-video-3194',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3189', 'HOMES-3194'],
            tags: ['food:Topic:Trend', 'food:Topic:Profile', 'food:Topic:Gardening 101', 'food:Homes navigation:DIY'],
            source: 'Australian House and Garden',
            sortOrder: '3',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Gardening 101',
                    urlName: 'gardening-101',
                    fullName: 'food_Topic_Gardening_101',
                    displayName: 'Gardening 101'
                },
                {
                    name: 'food:Topic:Profile',
                    urlName: 'profile',
                    fullName: 'food_Topic_Profile',
                    displayName: 'Profile'
                },
                {
                    name: 'food:Topic:Trend',
                    urlName: 'trend',
                    fullName: 'food_Topic_Trend',
                    displayName: 'Trend'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                }
            ],
            contentMonth: 11,
            contentTitle: 'TEST ME Article Long Title - How to get black and white decorating right Long Title Long Title EOM',
            contentVideo: {
                id: '4925012401001',
                name: 'Chrissy Teigan Snapchat',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4925012401001',
                        statusCode: 200,
                        brightcoveId: '4925012401001',
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4925012168001_4925012401001-th.jpg?pubId=761709621001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4925010520001_4925012401001-vs.jpg?pubId=761709621001'
                    }
                }
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                'Article Short Teaser - Find out how to use a combination of black and white to striking effect in your home. Short Teaser EOM',
            contentHasVideo: true,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-11-09T04:37:39.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageCaption: 'This is hero image caption Black brings a sense of drama to your home. Photo: Felix Forest'
        },
        {
            id: 'HOMES-3193',
            url: '/test-article-3193',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3189', 'HOMES-3193'],
            tags: [
                'food:Topic:Trend',
                'food:Topic:Profile',
                'people:profession:Designer',
                'food:Building:Type:House',
                'fashion:fashion_designer:Alexander McQueen',
                'food:Homes navigation:DIY'
            ],
            source: 'Australian House and Garden',
            sortOrder: '2',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'fashion:fashion_designer:Alexander McQueen',
                    urlName: 'alexander-mcqueen',
                    fullName: 'fashion_fashion_designer_Alexander_McQueen',
                    displayName: 'Alexander McQueen'
                },
                {
                    name: 'food:Topic:Profile',
                    urlName: 'profile',
                    fullName: 'food_Topic_Profile',
                    displayName: 'Profile'
                },
                {
                    name: 'food:Topic:Trend',
                    urlName: 'trend',
                    fullName: 'food_Topic_Trend',
                    displayName: 'Trend'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'food:Building:Type:House',
                    urlName: 'house',
                    fullName: 'food_Building_Type_House',
                    displayName: 'House'
                },
                {
                    name: 'people:profession:Designer',
                    urlName: 'designer',
                    fullName: 'people_profession_Designer',
                    displayName: 'Designer'
                }
            ],
            contentMonth: 11,
            contentTitle: 'Article Long Title - How to get black and white decorating right Long Title Long Title EOM',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                'Article Short Teaser - Find out how to use a combination of black and white to striking effect in your home. Short Teaser EOM',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-11-09T00:46:55.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageCaption: 'This is hero image caption Black brings a sense of drama to your home. Photo: Felix Forest'
        },
        {
            id: 'HOMES-3188',
            url: '/test-tags-3188',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3189', 'HOMES-3188'],
            tags: ['food:Topic:Tips and advice', 'food:Homes navigation:DIY'],
            source: 'Belle',
            sortOrder: '1',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                }
            ],
            contentMonth: 11,
            contentTitle: 'test Tags title length test title length test title length test title length test title length testtitle length test',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'test Tags',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-11-01T05:42:21.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-3186',
            url: '/kevinhgallery-3186',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3185', 'HOMES-3186'],
            tags: [
                'business:industry:Interior design',
                'food:Homes navigation:My Ideal House',
                'food:Products, Furniture and Fittings:Accessories and decor:Fabrics',
                'food:Topic:Luxury home'
            ],
            source: 'Belle',
            sortOrder: '2',
            contentTags: [
                'business:industry:Interior design',
                'food:Homes navigation:My Ideal House',
                'food:Products, Furniture and Fittings:Accessories and decor:Fabrics',
                'food:Topic:Luxury home'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home',
                    fullName: 'food_Topic_Luxury_home',
                    displayName: 'Luxury home'
                },
                {
                    name: 'food:Products, Furniture and Fittings:Accessories and decor:Fabrics',
                    urlName: 'fabrics',
                    fullName: 'food_Products_Furniture_and_Fittings_Accessories_and_decor_Fabrics',
                    displayName: 'Fabrics'
                },
                {
                    name: 'business:industry:Interior design',
                    urlName: 'interior-design',
                    fullName: 'business_industry_Interior_design',
                    displayName: 'Interior design'
                },
                {
                    name: 'food:Homes navigation:My Ideal House',
                    urlName: 'my-ideal-house',
                    fullName: 'food_Homes_navigation_My_Ideal_House',
                    displayName: 'My Ideal House'
                }
            ],
            contentMonth: 10,
            contentTitle: 'Test Gallery',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Test Gallery',
            pageSearchTags: [
                'Interior design',
                'Interior-design',
                'interior-design',
                'interior design',
                'My Ideal House',
                'My-Ideal-House',
                'my-ideal-house',
                'my ideal house',
                'Fabrics',
                'fabrics',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home'
            ],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-10-16T23:49:15.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3184',
            url: '/kev-test-gallery-3184',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3185', 'HOMES-3184'],
            tags: [
                'celebrities:celebrity:Nicholas Hoult',
                'food:Garden/Outdoor:Plants:Palms',
                'food:Topic:Gardening 101',
                'food:Homes navigation:Outdoor'
            ],
            source: 'real living',
            sortOrder: '1',
            contentTags: [
                'celebrities:celebrity:Nicholas Hoult',
                'food:Garden/Outdoor:Plants:Palms',
                'food:Topic:Gardening 101',
                'food:Homes navigation:Outdoor'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Gardening 101',
                    urlName: 'gardening-101',
                    fullName: 'food_Topic_Gardening_101',
                    displayName: 'Gardening 101'
                },
                {
                    name: 'celebrities:celebrity:Nicholas Hoult',
                    urlName: 'nicholas-hoult',
                    fullName: 'celebrities_celebrity_Nicholas_Hoult',
                    displayName: 'Nicholas Hoult'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                },
                {
                    name: 'food:Garden/Outdoor:Plants:Palms',
                    urlName: 'palms',
                    fullName: 'food_GardenOutdoor_Plants_Palms',
                    displayName: 'Palms'
                }
            ],
            contentMonth: 10,
            contentTitle: 'Kev test gallery looonnnngg title',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Kev test gallery short teaser',
            pageSearchTags: [
                'Nicholas Hoult',
                'Nicholas-Hoult',
                'nicholas-hoult',
                'nicholas hoult',
                'Palms',
                'palms',
                'Gardening 101',
                'Gardening-101',
                'gardening-101',
                'gardening 101',
                'Outdoor',
                'outdoor'
            ],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-10-12T01:52:25.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        }
    ]
};
