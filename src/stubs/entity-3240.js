export default {
    id: 'HOMES-3240',
    url: '/abhishek-first-gallery-3240',
    path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-3241', 'HOMES-3242', 'HOMES-3240'],
    tags: ['food:Homes navigation:Bedrooms', 'food:Topic:Kitchen news'],
    level: 5,
    nodeId: 'HOMES-3240',
    source: 'Belle',
    status: 'published',
    siteUrl: 'http://homes-site-au.sit.bxm.net.au',
    urlName: 'abhishek-first-gallery',
    nodeName: 'Abhishek First Gallery',
    parentID: 'HOMES-3242',
    siteCode: '56421E16-B4AB-417C-9645-A6A89644D4B4',
    siteName: 'homes',
    sortOrder: 1,
    contentBody: [
        {
            type: 'paragraph',
            label: 'Paragraph',
            content: 'Beauty inspo for school!'
        },
        {
            type: 'paragraph',
            label: 'Paragraph',
            content:
                'Test the second paragraph This is link Test the second paragraph Test the second paragraph Test the second paragraph \\n\\ntest bold text Test test Italic text the bold and italic text second paragraph Test the second paragraph Test the second paragraph Test the second paragraph Test the second paragraph Test the second paragraph Test the second paragraph Test the second paragraph Test the second paragraph \\n\\n## Test heading\\n test bullet 1\\n test bullet 2'
        }
    ],
    contentTags: ['food:Homes navigation:Bedrooms', 'food:Topic:Kitchen news'],
    contentYear: 2017,
    navIsHidden: 0,
    navRedirect: {
        Url: null,
        Mode: 2,
        Title: '',
        NodeId: null,
        NewWindow: false
    },
    tagsDetails: [
        {
            name: 'food:Homes navigation:Bedrooms',
            urlName: 'bedrooms',
            fullName: 'food_Homes_navigation_Bedrooms',
            displayName: 'Bedrooms '
        },
        {
            name: 'food:Topic:Kitchen news',
            urlName: 'kitchen-news',
            fullName: 'food_Topic_Kitchen_news',
            displayName: 'Kitchen news'
        }
    ],
    contentMonth: 2,
    contentTitle: 'This is long title',
    contentVideo: '',
    ampPreviewUrl: 'http://homes-site-au.sit.bxm.net.au/preview/amp/abhishek-first-gallery-3240',
    contentAuthor: [],
    nodeTypeAlias: 'Gallery',
    contentAuthors: [],
    contentGallery: [
        {
            url: 'http://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/Elle/2015/02/19/52893/benedict.jpg',
            open: false,
            tags: [],
            title: 'Jellyfish.jpg',
            valid: true,
            source: '',
            caption:
                'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus interdum odio ante, pulvinar egestas ligula scelerisque sit amet. Nunc ut tellus quis augue feugiat sodales. Vivamus vel pretium lorem. Morbi at quam quis nunc feugiat sodales nec vitae nulla. Sed ultrices porta bibendum. Pellentesque nisi nunc, efficitur sit amet nulla id, volutpat lobortis nisl. Maecenas auctor pulvinar molestie. Cras vitae tortor egestas, suscipit dolor vitae, tincidunt nisl.',
            credits: [],
            mediaType: 'image'
        },
        {
            url: 'http://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/Elle/2015/02/05/51410/resized.jpg',
            open: false,
            tags: [],
            title: '',
            valid: true,
            source: '',
            caption:
                'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus interdum odio ante, pulvinar egestas ligula scelerisque sit amet. Nunc ut tellus quis augue feugiat sodales. Vivamus vel pretium lorem. Morbi at quam quis nunc feugiat sodales nec vitae nulla. Sed ultrices porta bibendum. Pellentesque nisi nunc, efficitur sit amet nulla id, volutpat lobortis nisl. Maecenas auctor pulvinar molestie. Cras vitae tortor egestas, suscipit dolor vitae, tincidunt nisl.',
            credits: [],
            mediaType: 'image'
        },
        {
            open: false,
            valid: true,
            ispoen: false,
            videoId: '4127515677001',
            mediaType: 'video',
            videoTitle: '',
            brightcoveId: '4127515677001'
        },
        {
            url: 'http://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/Elle/2015/02/04/51358/blake-lively.jpg',
            open: false,
            tags: [],
            title: '',
            valid: true,
            source: '',
            caption:
                'Textured stone platforms in the living room, reminiscent of volcanic stone found on nearby beaches, act as informal seating. Limited-edition woven dining chairs, prismatic cubed side tables and the ‘Smock’ armchair by [Patricia Urquiola](http://www.patriciaurquiola.com/|target=\\"_blank\\"|rel=\\"nofollow\\"). Textured handwoven rugs, found on holiday in Tropea, Calabria, inject just the right amount of colour and pattern.',
            credits: [],
            mediaType: 'image'
        },
        {
            url: 'http://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/Elle/2016/12/05/107325/gigi-kendall-victoria-s-secret.jpg',
            open: false,
            tags: [],
            title: '',
            valid: true,
            source: '',
            caption: 'Sunburnt Country sculpture by Piers Buxton, is displayed on the fireplace’s floating stone platform.',
            credits: [],
            mediaType: 'image'
        },
        {
            url: 'http://assets.cougar.nineentertainment.com.au/Assets/Elle/2014/03/21/30477/montana-lead.jpg',
            open: false,
            tags: [],
            title: '',
            valid: true,
            source: '',
            caption:
                'The pendant lights above the dining table are a custom design by [Anchor Ceramics](http://anchorceramics.com/|target=\\"_blank\\"|rel=\\"nofollow\\"). The overscale barn door was designed by Ljiljana and separates the guest quarters from the dining and entry.',
            credits: [],
            mediaType: 'image'
        },
        {
            url: 'http://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/Elle/2015/07/22/63561/taylor-swift-clothing-line.jpg',
            open: false,
            tags: [],
            title: '',
            valid: true,
            source: '',
            caption:
                'Exfoliated granite benchtop and timber veneer cupboards in the kitchen create a naturally sleek profile. The ‘Vela’ outdoor chairs are from [Space](http://www.spacefurniture.com.au/|target=\\"_blank\\"|rel=\\"nofollow\\").',
            credits: [],
            mediaType: 'image'
        },
        {
            url: 'http://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/Elle/2015/07/15/63103/mainel0815_Brittany-06-070-FINAL.jpg',
            open: true,
            tags: [],
            title: '',
            valid: true,
            source: '',
            caption:
                'The bespoke wardrobe was custom designed by Ljiljana. The Belgian linen duvet and knitted leather throw add a soft touch, while the handwoven rug bought in Calabria, injects colour.',
            credits: [],
            mediaType: 'image'
        }
    ],
    contentSummary: 'it is a teaser',
    moduleParentId: 'HOMES-3240',
    pageSearchTags: ['Bedrooms', 'bedrooms', 'Kitchen news', 'Kitchen-news', 'kitchen-news', 'kitchen news'],
    contentCampaign: [],
    contentHasVideo: true,
    contentImageUrl: 'http://dev.assets.cougar.bauer-media.net.au/s3/digital-cougar-assets-dev/homes/2017/02/13/3240/Desert.jpg',
    nodeDateIndexed: '2018-08-17T10:47:15.61Z',
    pageDateCreated: '2017-02-13T02:28:52.00Z',
    contentImageSize: {
        type: 'jpg',
        width: 1024,
        height: 768
    },
    nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
    isAllAmpCompatible: true,
    contentAccessLevels: -1,
    contentSummaryTitle: 'short title',
    pageExcludeFromSitemap: 0,
    contentFacebookImageUrl: {
        tags: [],
        source: '',
        credits: []
    },
    pageExcludeDescendantsFromSitemap: 0,
    updated_at: '2018-08-17T10:47:16.089Z'
};
