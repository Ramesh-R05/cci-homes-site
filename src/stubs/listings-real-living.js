export default {
    totalCount: 66,
    data: [
        {
            id: 'HOMES-3240',
            url: '/abhishek-first-gallery-3240',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-3241', 'HOMES-3242', 'HOMES-3240'],
            tags: ['food:Homes navigation:Bedroom', 'food:Topic:Kitchen news'],
            source: 'real living',
            sortOrder: '1',
            contentTags: ['food:Homes navigation:Bedroom', 'food:Topic:Kitchen news'],
            contentYear: 2017,
            tagsDetails: [
                {
                    name: 'food:Topic:Kitchen news',
                    urlName: 'kitchen-news',
                    fullName: 'food_Topic_Kitchen_news',
                    displayName: 'Kitchen news'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                }
            ],
            contentMonth: 2,
            contentTitle: 'This is long title',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'it is a teaser',
            pageSearchTags: ['Bedroom', 'bedroom', 'Kitchen news', 'Kitchen-news', 'kitchen-news', 'kitchen news'],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2017-02-13T02:28:52.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3212',
            url: '/tags-test-3212',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-3212'],
            tags: ['tv:tv_network:TV1', 'food:Cooking time:More than 1 hour', 'food:Topic:In focus', 'food:Homes navigation:Bedroom'],
            source: 'real living',
            sortOrder: '4',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'tv:tv_network:TV1',
                    urlName: 'tv1',
                    fullName: 'tv_tv_network_TV1',
                    displayName: 'TV1'
                },
                {
                    name: 'food:Cooking time:More than 1 hour',
                    urlName: 'more-than-1-hour',
                    fullName: 'food_Cooking_time_More_than_1_hour',
                    displayName: 'More than 1 hour'
                },
                {
                    name: 'food:Topic:In focus',
                    urlName: 'in-focus',
                    fullName: 'food_Topic_In_focus',
                    displayName: 'In focus'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                }
            ],
            contentMonth: 12,
            contentTitle: 'tags test article',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Light and casual or dark and dramatic, this hard-working fabric does double duty in chic home interiors.',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-12-13T04:06:46.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-2785',
            url: '/halcyon-house-2785',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3207', 'HOMES-2785'],
            tags: ['food:Location and setting:Setting:Beachside', 'food:Topic:Design news', 'food:Homes navigation:DIY'],
            source: 'real living',
            sortOrder: '1',
            contentTags: ['food:Location and setting:Setting:Beachside', 'food:Topic:Design news', 'food:Homes navigation:DIY'],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Design news',
                    urlName: 'design-news',
                    fullName: 'food_Topic_Design_news',
                    displayName: 'Design news'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'food:Location and setting:Setting:Beachside',
                    urlName: 'beachside',
                    fullName: 'food_Location_and_setting_Setting_Beachside',
                    displayName: 'Beachside'
                }
            ],
            contentMonth: 12,
            contentTitle: 'Escape to Halcyon House',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'This luxurious boutique beachside hotel maintains a low key Aussie feel.',
            pageSearchTags: ['Beachside', 'beachside', 'Design news', 'Design-news', 'design-news', 'design news', 'DIY', 'diy'],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/07/2785/HERO_Halycon.jpg',
            pageDateCreated: '2016-12-01T13:00:00.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Halcyon House Hotel ',
            contentImageCaption: 'Halcyon House Hotel Cabarita Beach '
        },
        {
            id: 'HOMES-3184',
            url: '/kev-test-gallery-3184',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3185', 'HOMES-3184'],
            tags: [
                'celebrities:celebrity:Nicholas Hoult',
                'food:Garden/Outdoor:Plants:Palms',
                'food:Topic:Gardening 101',
                'food:Homes navigation:Outdoor'
            ],
            source: 'real living',
            sortOrder: '1',
            contentTags: [
                'celebrities:celebrity:Nicholas Hoult',
                'food:Garden/Outdoor:Plants:Palms',
                'food:Topic:Gardening 101',
                'food:Homes navigation:Outdoor'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Gardening 101',
                    urlName: 'gardening-101',
                    fullName: 'food_Topic_Gardening_101',
                    displayName: 'Gardening 101'
                },
                {
                    name: 'celebrities:celebrity:Nicholas Hoult',
                    urlName: 'nicholas-hoult',
                    fullName: 'celebrities_celebrity_Nicholas_Hoult',
                    displayName: 'Nicholas Hoult'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                },
                {
                    name: 'food:Garden/Outdoor:Plants:Palms',
                    urlName: 'palms',
                    fullName: 'food_GardenOutdoor_Plants_Palms',
                    displayName: 'Palms'
                }
            ],
            contentMonth: 10,
            contentTitle: 'Kev test gallery looonnnngg title',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Kev test gallery short teaser',
            pageSearchTags: [
                'Nicholas Hoult',
                'Nicholas-Hoult',
                'nicholas-hoult',
                'nicholas hoult',
                'Palms',
                'palms',
                'Gardening 101',
                'Gardening-101',
                'gardening-101',
                'gardening 101',
                'Outdoor',
                'outdoor'
            ],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-10-12T01:52:25.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        },
        {
            id: 'HOMES-3175',
            url: '/staging-top-50-rooms-3175',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3077', 'HOMES-3175'],
            tags: ['food:Topic:Renovating 101', 'food:Topic:Luxury home', 'Renovating:Materials:Cane', 'food:Homes navigation:DIY'],
            source: 'real living',
            sortOrder: '79',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home',
                    fullName: 'food_Topic_Luxury_home',
                    displayName: 'Luxury home'
                },
                {
                    name: 'food:Topic:Renovating 101',
                    urlName: 'renovating-101',
                    fullName: 'food_Topic_Renovating_101',
                    displayName: 'Renovating 101'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'Renovating:Materials:Cane',
                    urlName: 'cane',
                    fullName: 'Renovating_Materials_Cane',
                    displayName: 'Cane'
                }
            ],
            contentMonth: 8,
            contentTitle: 'Test Top 50 Rooms Staging Link',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'short teaser goes here',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            pageDateCreated: '2016-08-04T06:11:13.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-3064',
            url: '/pittwater-beach-shack-gets-a-heartfelt-renovation-3064',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3051', 'HOMES-3064'],
            tags: [
                'tv:tv_subject:Renovation',
                'food:Topic:Renovating 101',
                'food:Location and setting:Australia:New South Wales:Northern beaches',
                'food:Location and setting:Australia:New South Wales:Sydney:Sydney northern beaches',
                'food:Decorating:Style:Coastal/beach',
                'food:Homes navigation:DIY'
            ],
            source: 'real living',
            sortOrder: '4',
            contentTags: [
                'tv:tv_subject:Renovation',
                'food:Topic:Renovating 101',
                'food:Location and setting:Australia:New South Wales:Northern beaches',
                'food:Location and setting:Australia:New South Wales:Sydney:Sydney northern beaches',
                'food:Decorating:Style:Coastal/beach',
                'food:Homes navigation:DIY'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Renovating 101',
                    urlName: 'renovating-101',
                    fullName: 'food_Topic_Renovating_101',
                    displayName: 'Renovating 101'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    displayName: 'Coastal/beach'
                },
                {
                    name: 'food:Location and setting:Australia:New South Wales:Northern beaches',
                    urlName: 'northern-beaches',
                    fullName: 'food_Location_and_setting_Australia_New_South_Wales_Northern_beaches',
                    displayName: 'Northern beaches'
                },
                {
                    name: 'food:Location and setting:Australia:New South Wales:Sydney:Sydney northern beaches',
                    urlName: 'sydney-northern-beaches',
                    fullName: 'food_Location_and_setting_Australia_New_South_Wales_Sydney_Sydney_northern_beaches',
                    displayName: 'Sydney northern beaches'
                }
            ],
            contentMonth: 3,
            contentTitle: "Deb Bibby's Pittwater beach shack gets a heartfelt renovation",
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Editor-in-chief of *real living* Deb Bibby shares the highs and lows of her incredible renovation journey.',
            pageSearchTags: [
                'Renovation',
                'renovation',
                'Renovating 101',
                'Renovating-101',
                'renovating-101',
                'renovating 101',
                'Northern beaches',
                'Northern-beaches',
                'northern-beaches',
                'northern beaches',
                'Sydney northern beaches',
                'Sydney-northern-beaches',
                'sydney-northern-beaches',
                'sydney northern beaches',
                'Coastal/beach',
                'coastal/beach',
                'DIY',
                'diy'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/03/03/3064/Deb-HERO.jpg',
            pageDateCreated: '2016-03-03T00:15:29.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Pittwater beach shack gets a heartfelt renovation',
            contentImageCaption:
                '"Our new home has changed the way we live and how we feel. It’s our refuge and happy shelter," says Deb. Photo: Maree Homer'
        },
        {
            id: 'HOMES-3063',
            url: '/meet-modern-bohemian-cassandra-langbien-of-tribe-and-co-3063',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3049', 'HOMES-3063'],
            tags: ['common:topic:win', 'food:Homes navigation:Outdoor', 'food:Topic:Tips and advice'],
            source: 'real living',
            sortOrder: '9',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'common:topic:win',
                    urlName: 'win',
                    fullName: 'common_topic_win',
                    displayName: 'win'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                }
            ],
            contentMonth: 3,
            contentTitle: 'Meet “modern bohemian” Cassandra Langbien of Tribe & Co',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                'After travelling and living a “gypsy life” for the past 12 months, Cassandra is excited about her return to Australian and the chance to decorate her own place.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/03/03/3063/Cassandr-HERO.jpg',
            pageDateCreated: '2016-03-02T23:34:58.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'Meet “modern bohemian” Cassandra Langbien of Tribe & Co',
            contentImageCaption:
                'Cassandra Langbien’s homewares brand [Tribe & Co](http://tribeandcoliving.com/|target="_blank") has an emphasis on quality and uniqueness. Photo: Maree Homer'
        },
        {
            id: 'HOMES-3055',
            url: '/tropical-gardening-by-the-beach-3055',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3049', 'HOMES-3055'],
            tags: [
                'food:Garden/Outdoor:Garden style:Tropical garden',
                'food:Garden/Outdoor:Garden style:Coastal garden',
                'food:Topic:In focus',
                'food:Homes navigation:Outdoor'
            ],
            source: 'real living',
            sortOrder: '4',
            contentYear: 2016,
            sponsorName: 'Aquatest',
            tagsDetails: [
                {
                    name: 'food:Garden/Outdoor:Garden style:Coastal garden',
                    urlName: 'coastal-garden',
                    fullName: 'food_GardenOutdoor_Garden_style_Coastal_garden',
                    displayName: 'Coastal garden'
                },
                {
                    name: 'food:Garden/Outdoor:Garden style:Tropical garden',
                    urlName: 'tropical-garden',
                    fullName: 'food_GardenOutdoor_Garden_style_Tropical_garden',
                    displayName: 'Tropical garden'
                },
                {
                    name: 'food:Topic:In focus',
                    urlName: 'in-focus',
                    fullName: 'food_Topic_In_focus',
                    displayName: 'In focus'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                }
            ],
            contentMonth: 3,
            contentTitle: 'Tropical gardening by the beach TEST123',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'A Sydney Northern Beaches garden has grown into a lush piece of paradise. - JaniceTest',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/03/01/3055/hero2_150520H-garden-0232.jpg',
            pageDateCreated: '2016-03-01T04:39:10.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'coastal garden ideas',
            contentImageCaption: 'Timber pots, made from old Thai ox carts, are filled with succulents. Photo: James Henry'
        },
        {
            id: 'HOMES-3050',
            url: '/sydney-apartment-gets-swedish-summer-house-makeover-3050',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3051', 'HOMES-3050'],
            tags: [
                'food:Decorating:Style:Scandinavian style',
                'food:Topic:Feature home',
                'food:Decorating:Style:Contemporary',
                'food:Homes navigation:Real Homes'
            ],
            source: 'real living',
            sortOrder: '1',
            contentTags: [
                'food:Decorating:Style:Scandinavian style',
                'food:Topic:Feature home',
                'food:Decorating:Style:Contemporary',
                'food:Homes navigation:Real Homes'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Feature home',
                    urlName: 'feature-home',
                    fullName: 'food_Topic_Feature_home',
                    displayName: 'Feature home'
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                },
                {
                    name: 'food:Decorating:Style:Scandinavian style',
                    urlName: 'scandinavian-style',
                    fullName: 'food_Decorating_Style_Scandinavian_style',
                    displayName: 'Scandinavian style'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Sydney apartment gets Swedish summer house makeoverTEST',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: '“It almost merges and marries mid-century Scandi with a beachside Australian feel,” says owner Fräg Woodall.',
            pageSearchTags: [
                'Scandinavian style',
                'Scandinavian-style',
                'scandinavian-style',
                'scandinavian style',
                'Feature home',
                'Feature-home',
                'feature-home',
                'feature home',
                'Contemporary',
                'contemporary',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/03/01/3050/Scandi-HERO.jpg',
            pageDateCreated: '2016-02-29T23:02:46.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Sydney apartment gets Swedish summer house makeover'
        },
        {
            id: 'HOMES-3012',
            url: '/how-to-hide-your-tv-3012',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-3012'],
            tags: [
                '"food:Products, Furniture and Fittings:Large appliances:Televisions:Concealed television"',
                'food:Topic:Tips and advice',
                'food:Room:Living:Living rooms',
                '"food:Products, Furniture and Fittings:Large appliances:Televisions:Wall mounted television"',
                'food:Homes navigation:Living'
            ],
            source: 'real living',
            sortOrder: '58',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:Living',
                    urlName: 'living',
                    fullName: 'food_Homes_navigation_Living',
                    displayName: 'Living'
                },
                {
                    name: 'food:Room:Living:Living rooms',
                    urlName: 'living-rooms',
                    fullName: 'food_Room_Living_Living_rooms',
                    displayName: 'Living rooms'
                }
            ],
            contentMonth: 2,
            contentTitle: 'How to hide your TV',
            contentVideo: {
                id: '4766735511001',
                name: 'How to hide your TV ',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4766735511001',
                        statusCode: 200,
                        brightcoveId: '4766735511001',
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4766739805001_4766735511001-th.jpg?pubId=761709621001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4766863342001_4766735511001-vs.jpg?pubId=761709621001'
                    }
                }
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Stylist Jackie Brown on how to hide your television in plain sight.',
            contentHasVideo: true,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/22/3012/TV-HERO.jpg',
            pageDateCreated: '2016-02-22T00:39:38.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'How to hide your TV'
        },
        {
            id: 'HOMES-3008',
            url: '/pretty-places-to-rest-your-head-3008',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-3008'],
            tags: [
                'food:Room:Bedroom:Bedrooms',
                'food:Topic:Inspired by',
                'food:Room:Bedroom:Master bedroom',
                '"food:Products, Furniture and Fittings:Linens:Bedding:Bedlinen"',
                'food:Decorating:Style:Contemporary',
                'food:Decorating:Style:Urban',
                '"food:Products, Furniture and Fittings:Furniture and fittings by room:Bedroom furniture and decor:Beds:Bed frames"',
                'food:Homes navigation:Bedroom'
            ],
            source: 'real living',
            sortOrder: '56',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Inspired by',
                    urlName: 'inspired-by',
                    fullName: 'food_Topic_Inspired_by',
                    displayName: 'Inspired by'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                },
                {
                    name: 'food:Decorating:Style:Urban',
                    urlName: 'urban',
                    fullName: 'food_Decorating_Style_Urban',
                    displayName: 'Urban'
                },
                {
                    name: 'food:Room:Bedroom:Bedrooms',
                    urlName: 'bedrooms',
                    fullName: 'food_Room_Bedroom_Bedrooms',
                    displayName: 'Bedrooms'
                },
                {
                    name: 'food:Room:Bedroom:Master bedroom',
                    urlName: 'master-bedroom',
                    fullName: 'food_Room_Bedroom_Master_bedroom',
                    displayName: 'Master bedroom'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Pretty places to rest your head',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                'Watch a fun video of how stylist Sarah Ellison put together the bedroom shoot featured on the March cover of *real living*.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/19/3008/hero_C14B9F62-89.jpg',
            pageDateCreated: '2016-02-19T02:07:25.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'nautical bedroom',
            contentImageCaption:
                '*real living* for Freedom "Sketches" quilt cover set in Blue, $169 for queen, [Freedom](http://www.freedom.com.au/?utm_campaign=supplier/|target="_blank"). '
        },
        {
            id: 'HOMES-3001',
            url: '/kitchen-renovation-with-a-flexible-layout-3001',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-3001'],
            tags: [
                'food:Room:Kitchen:Kitchens',
                'food:Topic:Feature room',
                'food:Renovating:Materials:Tiles:Subway tiles',
                'food:Renovating:Materials:Tiles:Decorative tiles',
                'food:Decorating:Colour:White',
                "food:Products, Furniture and Fittings:Furniture and fittings by room:Kitchen furniture and products:Island bench/ butcher's block",
                'food:Renovating:Trims and fittings:Joinery',
                'food:Decorating:Colour:Yellow',
                'food:Homes navigation:Kitchen'
            ],
            source: 'real living',
            sortOrder: '51',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Feature room',
                    urlName: 'feature-room',
                    fullName: 'food_Topic_Feature_room',
                    displayName: 'Feature room'
                },
                {
                    name: 'food:Renovating:Materials:Tiles:Subway tiles',
                    urlName: 'subway-tiles',
                    fullName: 'food_Renovating_Materials_Tiles_Subway_tiles',
                    displayName: 'Subway tiles'
                },
                {
                    name: 'food:Renovating:Materials:Tiles:Decorative tiles',
                    urlName: 'decorative-tiles',
                    fullName: 'food_Renovating_Materials_Tiles_Decorative_tiles',
                    displayName: 'Decorative tiles'
                },
                {
                    name: 'food:Renovating:Trims and fittings:Joinery',
                    urlName: 'joinery',
                    fullName: 'food_Renovating_Trims_and_fittings_Joinery',
                    displayName: 'Joinery'
                },
                {
                    name: 'food:Homes navigation:Kitchen',
                    urlName: 'kitchen',
                    fullName: 'food_Homes_navigation_Kitchen',
                    displayName: 'Kitchen'
                },
                {
                    name:
                        "food:Products, Furniture and Fittings:Furniture and fittings by room:Kitchen furniture and products:Island bench/ butcher's block",
                    urlName: 'island-bench-butchers-block',
                    fullName:
                        'food_Products_Furniture_and_Fittings_Furniture_and_fittings_by_room_Kitchen_furniture_and_products_Island_bench_butchers_block',
                    displayName: "Island bench/ butcher's block"
                },
                {
                    name: 'food:Decorating:Colour:White',
                    urlName: 'white',
                    fullName: 'food_Decorating_Colour_White',
                    displayName: 'White'
                },
                {
                    name: 'food:Room:Kitchen:Kitchens',
                    urlName: 'kitchens',
                    fullName: 'food_Room_Kitchen_Kitchens',
                    displayName: 'Kitchens'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Kitchen renovation with a moveable layout TEST',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'A flexible attitude has transformed this spatially challenged kitchen in a Sydney heritage home.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/19/3001/hero_C14BA322-Kitchen-056.jpg',
            pageDateCreated: '2016-02-18T04:54:29.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'white kitchen ideas',
            contentImageCaption:
                '"The key to the success of this kitchen was all in the space planning, joinery design and material selection." - Emma Rees-Raaijmakers, [Arque](http://www.arque.com.au/?utm_campaign=supplier/|target="_blank")'
        },
        {
            id: 'HOMES-2985',
            url: '/easy-fix-fun-ideas-update-your-rental-space-2985',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2985'],
            tags: [
                'food:DIY and craft:How-to:Decorating',
                'food:Topic:Tips and advice',
                'food:Room:Bedroom:Bedrooms',
                'food:Room:Bathroom:Bathrooms',
                'food:Homes navigation:Bathroom & Laundry'
            ],
            source: 'real living',
            sortOrder: '44',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:DIY and craft:How-to:Decorating',
                    urlName: 'decorating',
                    fullName: 'food_DIY_and_craft_How_to_Decorating',
                    displayName: 'Decorating'
                },
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:Bathroom & Laundry',
                    urlName: 'bathroom-and-laundry',
                    fullName: 'food_Homes_navigation_Bathroom_and_Laundry',
                    displayName: 'Bathroom & Laundry'
                },
                {
                    name: 'food:Room:Bathroom:Bathrooms',
                    urlName: 'bathrooms',
                    fullName: 'food_Room_Bathroom_Bathrooms',
                    displayName: 'Bathrooms'
                },
                {
                    name: 'food:Room:Bedroom:Bedrooms',
                    urlName: 'bedrooms',
                    fullName: 'food_Room_Bedroom_Bedrooms',
                    displayName: 'Bedrooms'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Easy fix: fun ideas update your rental space',
            contentVideo: {
                id: '4757454973001',
                name: 'Easy fix: fun ideas update your rental space',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4757454973001',
                        statusCode: 200,
                        brightcoveId: '4757454973001',
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4757476896001_4757454973001-th.jpg?pubId=761709621001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4757489627001_4757454973001-vs.jpg?pubId=761709621001'
                    }
                }
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Stylist Jackie Brown shares her tips for making your rental property feel like home.',
            contentHasVideo: true,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/16/2985/Renters-HERO-FINAL.jpg',
            pageDateCreated: '2016-02-15T22:59:09.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'Easy fix: fun ideas update your rental space'
        },
        {
            id: 'HOMES-2972',
            url: '/true-confessions-of-a-renovation-2972',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2972'],
            tags: [
                'food:Renovating:Renovation type:Restoration',
                'food:Topic:Renovating 101',
                'food:Decorating:Style:Coastal/beach',
                'food:Homes navigation:DIY'
            ],
            source: 'real living',
            sortOrder: '36',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Renovating 101',
                    urlName: 'renovating-101',
                    fullName: 'food_Topic_Renovating_101',
                    displayName: 'Renovating 101'
                },
                {
                    name: 'food:Renovating:Renovation type:Restoration',
                    urlName: 'restoration',
                    fullName: 'food_Renovating_Renovation_type_Restoration',
                    displayName: 'Restoration'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    displayName: 'Coastal/beach'
                }
            ],
            contentMonth: 2,
            contentTitle: 'True confessions of a renovation',
            contentVideo: {
                id: '4751359560001',
                name: 'True Confessions Of A Renovation',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4751359560001',
                        statusCode: 200,
                        brightcoveId: '4751359560001',
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4751460045001_4751359560001-th.jpg?pubId=761709621001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4751368927001_Poster-facebook-youtube.jpg?pubId=761709621001'
                    }
                }
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                'This simple beach shack on the shores of Pittwater, owned by Deb Bibby, editor-in-chief of *real living*, underwent a stunning transformation.',
            contentHasVideo: true,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/15/2972/Deb-Hero.jpg',
            pageDateCreated: '2016-02-12T03:33:43.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageCaption:
                'Renovating will daunt even the bravest of hearts and produce more anxiety than riding a wild horse. Deb Bibby, editor-in-chief of real living, reveals all (including her heart on her sleeve). Photo: Maree Homer '
        },
        {
            id: 'HOMES-2968',
            url: '/handy-woman-diy-wall-art-2968',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2968'],
            tags: [
                'food:Products, Furniture and Fittings:Accessories and decor',
                'food:Topic:DIY Projects',
                'food:Renovating:Trims and fittings:Walls & paint',
                'food:Renovating:Materials:Wallpaper',
                'food:Homes navigation:DIY'
            ],
            source: 'real living',
            sortOrder: '33',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Renovating:Materials:Wallpaper',
                    urlName: 'wallpaper',
                    fullName: 'food_Renovating_Materials_Wallpaper',
                    displayName: 'Wallpaper'
                },
                {
                    name: 'food:Topic:DIY Projects',
                    urlName: 'diy-projects',
                    fullName: 'food_Topic_DIY_Projects',
                    displayName: 'DIY Projects'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Handy woman: DIY washi tape wall art',
            contentVideo: {
                id: '4751241250001',
                name: 'Washi tape wall decal',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4751241250001',
                        statusCode: 200,
                        brightcoveId: '4751241250001',
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4751273987001_4751241250001-th.jpg?pubId=761709621001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4755479680001_4751241250001-vs.jpg?pubId=761709621001'
                    }
                }
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Create a striking statement with a washi tape wall decal.',
            contentHasVideo: true,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/15/2968/Handy-Woman-Hero.jpg',
            pageDateCreated: '2016-02-12T00:54:18.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'Handy woman: DIY wall art',
            contentImageCaption: 'Update your interiors with this decorative DIY project. Photo: Brett Stevens | Styling: Corina Koch'
        },
        {
            id: 'HOMES-2963',
            url: '/shop-the-look-coastal-inspired-dining-2963',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2963'],
            tags: [
                'food:Decorating:Style:Coastal/beach',
                'food:Topic:Shopping',
                'food:Room:Dining:Dining rooms',
                'food:Products, Furniture and Fittings:Furniture and fittings by room:Outdoor furniture:Hanging chair',
                'food:Decorating:Colour:Blue:Bright blue',
                'food:Decorating:Style:Contemporary',
                'food:Season:Summer',
                'food:Homes navigation:Kitchen'
            ],
            source: 'real living',
            sortOrder: '22',
            contentTags: [
                'food:Decorating:Style:Coastal/beach',
                'food:Topic:Shopping',
                'food:Room:Dining:Dining rooms',
                'food:Products, Furniture and Fittings:Furniture and fittings by room:Outdoor furniture:Hanging chair',
                'food:Decorating:Colour:Blue:Bright blue',
                'food:Decorating:Style:Contemporary',
                'food:Season:Summer',
                'food:Homes navigation:Kitchen'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Shopping',
                    urlName: 'shopping',
                    fullName: 'food_Topic_Shopping',
                    displayName: 'Shopping'
                },
                {
                    name: 'food:Homes navigation:Kitchen',
                    urlName: 'kitchen',
                    fullName: 'food_Homes_navigation_Kitchen',
                    displayName: 'Kitchen'
                },
                {
                    name: 'food:Products, Furniture and Fittings:Furniture and fittings by room:Outdoor furniture:Hanging chair',
                    urlName: 'hanging-chair',
                    fullName: 'food_Products_Furniture_and_Fittings_Furniture_and_fittings_by_room_Outdoor_furniture_Hanging_chair',
                    displayName: 'Hanging chair'
                },
                {
                    name: 'food:Season:Summer',
                    urlName: 'summer',
                    fullName: 'food_Season_Summer',
                    displayName: 'Summer'
                },
                {
                    name: 'food:Decorating:Colour:Blue:Bright blue',
                    urlName: 'bright-blue',
                    fullName: 'food_Decorating_Colour_Blue_Bright_blue',
                    displayName: 'Bright blue'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                },
                {
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    displayName: 'Coastal/beach'
                },
                {
                    name: 'food:Room:Dining:Dining rooms',
                    urlName: 'dining-rooms',
                    fullName: 'food_Room_Dining_Dining_rooms',
                    displayName: 'Dining rooms'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Shop the look: Coastal-inspired dining',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: "Transform, revive, refresh with summer's hottest affordable buys.",
            pageSearchTags: [
                'Coastal/beach',
                'coastal/beach',
                'Shopping',
                'shopping',
                'Dining rooms',
                'Dining-rooms',
                'dining-rooms',
                'dining rooms',
                'Hanging chair',
                'Hanging-chair',
                'hanging-chair',
                'hanging chair',
                'Bright blue',
                'Bright-blue',
                'bright-blue',
                'bright blue',
                'Contemporary',
                'contemporary',
                'Summer',
                'summer',
                'Kitchen',
                'kitchen'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/11/2963/hero_C14A4502-120.jpg',
            pageDateCreated: '2016-02-11T06:37:50.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'coastal dining room ideas'
        },
        {
            id: 'HOMES-2959',
            url: '/aussie-olympian-wins-gold-on-home-reno-2959',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2959'],
            tags: [
                'food:Building:Type:House',
                'food:Topic:Feature home',
                'food:Building:Building style:Victorian',
                'food:Location and setting:Australia:Victoria:Melbourne:Melbourne inner city',
                'food:Renovating:Renovation type:Renovation',
                'food:Renovating:Renovation type:Extension',
                'food:Homes navigation:Real Homes'
            ],
            source: 'real living',
            sortOrder: '21',
            contentTags: [
                'food:Building:Type:House',
                'food:Topic:Feature home',
                'food:Building:Building style:Victorian',
                'food:Location and setting:Australia:Victoria:Melbourne:Melbourne inner city',
                'food:Renovating:Renovation type:Renovation',
                'food:Renovating:Renovation type:Extension',
                'food:Homes navigation:Real Homes'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Feature home',
                    urlName: 'feature-home',
                    fullName: 'food_Topic_Feature_home',
                    displayName: 'Feature home'
                },
                {
                    name: 'food:Renovating:Renovation type:Extension',
                    urlName: 'extension',
                    fullName: 'food_Renovating_Renovation_type_Extension',
                    displayName: 'Extension'
                },
                {
                    name: 'food:Renovating:Renovation type:Renovation',
                    urlName: 'renovation',
                    fullName: 'food_Renovating_Renovation_type_Renovation',
                    displayName: 'Renovation'
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                },
                {
                    name: 'food:Building:Building style:Victorian',
                    urlName: 'victorian',
                    fullName: 'food_Building_Building_style_Victorian',
                    displayName: 'Victorian'
                },
                {
                    name: 'food:Building:Type:House',
                    urlName: 'house',
                    fullName: 'food_Building_Type_House',
                    displayName: 'House'
                },
                {
                    name: 'food:Location and setting:Australia:Victoria:Melbourne:Melbourne inner city',
                    urlName: 'melbourne-inner-city',
                    fullName: 'food_Location_and_setting_Australia_Victoria_Melbourne_Melbourne_inner_city',
                    displayName: 'Melbourne inner city'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Aussie Olympian wins gold on home reno',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'A snowboarding champion brings determination and speed to the purchase and renovation of her melbourne home',
            pageSearchTags: [
                'House',
                'house',
                'Feature home',
                'Feature-home',
                'feature-home',
                'feature home',
                'Victorian',
                'victorian',
                'Melbourne inner city',
                'Melbourne-inner-city',
                'melbourne-inner-city',
                'melbourne inner city',
                'Renovation',
                'renovation',
                'Extension',
                'extension',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/11/2959/Lyle-House-01.jpg',
            pageDateCreated: '2016-02-11T01:18:07.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Johanna Lyle house'
        },
        {
            id: 'HOMES-2926',
            url: '/5-clever-money-moves-to-make-right-now-2926',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2926'],
            tags: [
                'internet:website_category:Budget',
                'food:Topic:Special report',
                'food:Topic:Tips and advice',
                'book:book_subject:Money',
                'food:Profiles:Finance expert',
                'food:Topic:How to',
                'food:Homes navigation:DIY'
            ],
            source: 'real living',
            sortOrder: '12',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Profiles:Finance expert',
                    urlName: 'finance-expert',
                    fullName: 'food_Profiles_Finance_expert',
                    displayName: 'Finance expert'
                },
                {
                    name: 'food:Topic:Special report',
                    urlName: 'special-report',
                    fullName: 'food_Topic_Special_report',
                    displayName: 'Special report'
                },
                {
                    name: 'food:Topic:How to',
                    urlName: 'how-to',
                    fullName: 'food_Topic_How_to',
                    displayName: 'How to'
                },
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'book:book_subject:Money',
                    urlName: 'money',
                    fullName: 'book_book_subject_Money',
                    displayName: 'Money'
                }
            ],
            contentMonth: 2,
            contentTitle: '5 smart money moves',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                "You may not have heard of these financial tips and tricks, but they are sure to give your finances a boost. *Money Magazine*'s Effie Zahos shares her advice.",
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/05/2926/201405093169825794.jpg',
            pageDateCreated: '2016-02-05T00:01:22.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: ' financial tips for 2016',
            contentImageCaption:
                'A few minor adjustments can make a big difference to the state of your finances - now and into the future. Photo: Will Horner / bauersyndication.com.au\r\n\r\n'
        },
        {
            id: 'HOMES-2920',
            url: '/give-your-bedroom-a-sartorial-edge-2920',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2920'],
            tags: [
                'food:Room:Bedroom:Bedrooms',
                'food:Topic:Shopping',
                'food:Renovating:Renovation type:Remodel',
                'food:Decorating:Style:Urban',
                'food:Decorating:Colour:Metallic:Copper',
                'food:Decorating:Colour:Pastel:Pastel pink',
                'food:Homes navigation:Bedroom'
            ],
            source: 'real living',
            sortOrder: '12',
            contentTags: [
                'food:Room:Bedroom:Bedrooms',
                'food:Topic:Shopping',
                'food:Renovating:Renovation type:Remodel',
                'food:Decorating:Style:Urban',
                'food:Decorating:Colour:Metallic:Copper',
                'food:Decorating:Colour:Pastel:Pastel pink',
                'food:Homes navigation:Bedroom'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Shopping',
                    urlName: 'shopping',
                    fullName: 'food_Topic_Shopping',
                    displayName: 'Shopping'
                },
                {
                    name: 'food:Renovating:Renovation type:Remodel',
                    urlName: 'remodel',
                    fullName: 'food_Renovating_Renovation_type_Remodel',
                    displayName: 'Remodel'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Decorating:Colour:Pastel:Pastel pink',
                    urlName: 'pastel-pink',
                    fullName: 'food_Decorating_Colour_Pastel_Pastel_pink',
                    displayName: 'Pastel pink'
                },
                {
                    name: 'food:Decorating:Style:Urban',
                    urlName: 'urban',
                    fullName: 'food_Decorating_Style_Urban',
                    displayName: 'Urban'
                },
                {
                    name: 'food:Room:Bedroom:Bedrooms',
                    urlName: 'bedrooms',
                    fullName: 'food_Room_Bedroom_Bedrooms',
                    displayName: 'Bedrooms'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Give your bedroom a sartorial edge',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Fashion and interiors can work perfectly together – and this girly glam bedroom proves it.',
            pageSearchTags: [
                'Bedrooms',
                'bedrooms',
                'Shopping',
                'shopping',
                'Remodel',
                'remodel',
                'Urban',
                'urban',
                'Copper',
                'copper',
                'Pastel pink',
                'Pastel-pink',
                'pastel-pink',
                'pastel pink',
                'Bedroom',
                'bedroom'
            ],
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/04/2920/hero_FashionHome_Bedroom2_main.jpg',
            pageDateCreated: '2016-02-04T01:13:29.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'girly bedroom ideas'
        },
        {
            id: 'HOMES-2901',
            url: '/rl-newsletter-competition-2901',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2901'],
            tags: [
                'food:Products, Furniture and Fittings:Large appliances:Computers',
                'food:Topic:Creative space',
                'food:Topic:Design news',
                'food:Homes navigation:Real Homes'
            ],
            source: 'real living',
            sortOrder: '3',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Creative space',
                    urlName: 'creative-space',
                    fullName: 'food_Topic_Creative_space',
                    displayName: 'Creative space'
                },
                {
                    name: 'food:Topic:Design news',
                    urlName: 'design-news',
                    fullName: 'food_Topic_Design_news',
                    displayName: 'Design news'
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                },
                {
                    name: 'food:Products, Furniture and Fittings:Large appliances:Computers',
                    urlName: 'computers',
                    fullName: 'food_Products_Furniture_and_Fittings_Large_appliances_Computers',
                    displayName: 'Computers'
                }
            ],
            contentMonth: 2,
            contentTitle: 'RL newsletter competition',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Win a handy portable WD Passport Ultra hard drive in beautiful Berry for yourself or your partner this Valentine’s Day.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/10/2901/WD_RL-comp-hero100216.jpg',
            pageDateCreated: '2016-02-01T00:50:17.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        }
    ]
};
