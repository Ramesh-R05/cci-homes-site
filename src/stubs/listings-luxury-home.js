export default {
    totalCount: 53,
    data: [
        {
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            contentMonth: 12,
            contentTitle: 'Template HTL Poll',
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-3210',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2016-12-08T05:04:01.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-3210'],
            sortOrder: '2',
            source: 'homes+',
            tags: ['food:Topic:Luxury home', 'food:Homes navigation:Bedroom'],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom'
                }
            ],
            url: '/template-htl-poll-3210'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Mornington Peninsular home',
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            contentMonth: 11,
            contentSummary:
                'A quiet place on the Mornington Peninsula to relax and recharge has also become a source of inspiration for these creative owners.',
            contentTags: [
                'food:Location and setting:Australia:Victoria:Mornington Peninsula',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Coastal/beach',
                'food:Homes navigation:Real Homes'
            ],
            contentTitle: 'A luxurious bushland retreat',
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-3201',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2016-11-23T03:00:27.00Z',
            pageSearchTags: [
                'Mornington Peninsula',
                'Mornington-Peninsula',
                'mornington-peninsula',
                'mornington peninsula',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Coastal/beach',
                'coastal/beach',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3196', 'HOMES-3201'],
            sortOrder: '3',
            source: 'Australian House and Garden',
            tags: [
                'food:Location and setting:Australia:Victoria:Mornington Peninsula',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Coastal/beach',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Coastal/beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach'
                },
                {
                    displayName: 'Mornington Peninsula',
                    fullName: 'food_Location_and_setting_Australia_Victoria_Mornington_Peninsula',
                    name: 'food:Location and setting:Australia:Victoria:Mornington Peninsula',
                    urlName: 'mornington-peninsula'
                }
            ],
            url: '/test-stub-gallery-3201'
        },
        {
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            contentMonth: 10,
            contentSummary: 'Test Gallery',
            contentTags: [
                'business:industry:Interior design',
                'food:Homes navigation:My Ideal House',
                'food:Products, Furniture and Fittings:Accessories and decor:Fabrics',
                'food:Topic:Luxury home'
            ],
            contentTitle: 'Test Gallery',
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-3186',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2016-10-16T23:49:15.00Z',
            pageSearchTags: [
                'Interior design',
                'Interior-design',
                'interior-design',
                'interior design',
                'My Ideal House',
                'My-Ideal-House',
                'my-ideal-house',
                'my ideal house',
                'Fabrics',
                'fabrics',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-3185', 'HOMES-3186'],
            sortOrder: '2',
            source: 'Belle',
            tags: [
                'business:industry:Interior design',
                'food:Homes navigation:My Ideal House',
                'food:Products, Furniture and Fittings:Accessories and decor:Fabrics',
                'food:Topic:Luxury home'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Fabrics',
                    fullName: 'food_Products_Furniture_and_Fittings_Accessories_and_decor_Fabrics',
                    name: 'food:Products, Furniture and Fittings:Accessories and decor:Fabrics',
                    urlName: 'fabrics'
                },
                {
                    displayName: 'Interior design',
                    fullName: 'business_industry_Interior_design',
                    name: 'business:industry:Interior design',
                    urlName: 'interior-design'
                },
                {
                    displayName: 'My Ideal House',
                    fullName: 'food_Homes_navigation_My_Ideal_House',
                    name: 'food:Homes navigation:My Ideal House',
                    urlName: 'my-ideal-house'
                }
            ],
            url: '/kevinhgallery-3186'
        },
        {
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131321242_hero150115---rl---shopping-3-timeless-spaces-063v0.5-1.jpg',
            contentMonth: 8,
            contentSummary: 'short teaser goes here',
            contentTitle: 'Test Top 50 Rooms Staging Link',
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-3175',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2016-08-04T06:11:13.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3077', 'HOMES-3175'],
            sortOrder: '79',
            source: 'real living',
            tags: ['food:Topic:Renovating 101', 'food:Topic:Luxury home', 'Renovating:Materials:Cane', 'food:Homes navigation:DIY'],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Renovating 101',
                    fullName: 'food_Topic_Renovating_101',
                    name: 'food:Topic:Renovating 101',
                    urlName: 'renovating-101'
                },
                {
                    displayName: 'DIY',
                    fullName: 'food_Homes_navigation_DIY',
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy'
                },
                {
                    displayName: 'Cane',
                    fullName: 'Renovating_Materials_Cane',
                    name: 'Renovating:Materials:Cane',
                    urlName: 'cane'
                }
            ],
            url: '/staging-top-50-rooms-3175'
        },
        {
            contentHasVideo: true,
            contentImageCaption:
                '“I make a lot of stuff, and I’m constantly making more stuff and filling the space with lots and lots of stuff,” says Jonathan.',
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/19/3009/hero_LGarcia_JonathanAdler_0657.jpg',
            contentMonth: 2,
            contentSummary: 'Dazzling colours and eccentric panache reign in the home of design’s king of pop.',
            contentTitle: "Jonathan Adler's Greenwich Village apartment",
            contentVideo: {
                id: '4763633999001',
                name: 'At Home With Jonathon Adler',
                properties: {
                    videoConfiguration: {
                        brightcoveId: '4763633999001',
                        status: '',
                        statusCode: 200,
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4763643515001_4763633999001-th.jpg?pubId=761709621001',
                        videoId: '4763633999001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4763644497001_4763633999001-vs.jpg?pubId=761709621001'
                    }
                },
                type: 'VideoItem'
            },
            contentYear: 2016,
            id: 'HOMES-3009',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2016-02-19T03:30:20.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-3009'],
            sortOrder: '57',
            source: 'Belle',
            tags: [
                'food:Decorating:Style:Luxe',
                'food:Topic:Luxury home',
                'food:Profiles:Designer',
                'food:Building:Type:Apartment/unit',
                'food:Decorating:Colour:Multicolour',
                'food:Building:Building style:Heritage-listed',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Multicolour',
                    fullName: 'food_Decorating_Colour_Multicolour',
                    name: 'food:Decorating:Colour:Multicolour',
                    urlName: 'multicolour'
                },
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Luxe',
                    fullName: 'food_Decorating_Style_Luxe',
                    name: 'food:Decorating:Style:Luxe',
                    urlName: 'luxe'
                },
                {
                    displayName: 'Heritage-listed',
                    fullName: 'food_Building_Building_style_Heritage_listed',
                    name: 'food:Building:Building style:Heritage-listed',
                    urlName: 'heritage-listed'
                },
                {
                    displayName: 'Apartment/unit',
                    fullName: 'food_Building_Type_Apartmentunit',
                    name: 'food:Building:Type:Apartment/unit',
                    urlName: 'apartment-unit'
                }
            ],
            url: '/jonathan-adlers-greenwich-village-apartment-3009'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Darren Palmer house',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/17/2993/hero_C14B9601-9772.jpg',
            contentMonth: 2,
            contentSummary:
                'Celebrity interior designer and TV renovation judge Darren Palmer works his magic off the screen to create a dreamy, durable Sydney family home.',
            contentTags: [
                'food:Profiles:Celebrity',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Classic',
                'food:Location and setting:Setting:Inner-city',
                'food:Building:Building style:Heritage-listed',
                'food:Renovating:Materials:Stone',
                'food:Renovating:Materials:Linen',
                'food:Decorating:Style:Hamptons',
                'food:Building:Type:Terrace',
                'food:Homes navigation:Real Homes'
            ],
            contentTitle: "Darren Palmer's contemporary-classic family home",
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-2993',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2016-02-17T05:04:56.00Z',
            pageSearchTags: [
                'Celebrity',
                'celebrity',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Classic',
                'classic',
                'Inner-city',
                'inner-city',
                'Heritage-listed',
                'heritage-listed',
                'Stone',
                'stone',
                'Linen',
                'linen',
                'Hamptons',
                'hamptons',
                'Terrace',
                'terrace',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2993'],
            sortOrder: '28',
            source: 'Australian House and Garden',
            tags: [
                'food:Profiles:Celebrity',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Classic',
                'food:Location and setting:Setting:Inner-city',
                'food:Building:Building style:Heritage-listed',
                'food:Renovating:Materials:Stone',
                'food:Renovating:Materials:Linen',
                'food:Decorating:Style:Hamptons',
                'food:Building:Type:Terrace',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Stone',
                    fullName: 'food_Renovating_Materials_Stone',
                    name: 'food:Renovating:Materials:Stone',
                    urlName: 'stone'
                },
                {
                    displayName: 'Linen',
                    fullName: 'food_Renovating_Materials_Linen',
                    name: 'food:Renovating:Materials:Linen',
                    urlName: 'linen'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Inner-city',
                    fullName: 'food_Location_and_setting_Setting_Inner_city',
                    name: 'food:Location and setting:Setting:Inner-city',
                    urlName: 'inner-city'
                },
                {
                    displayName: 'Classic',
                    fullName: 'food_Decorating_Style_Classic',
                    name: 'food:Decorating:Style:Classic',
                    urlName: 'classic'
                },
                {
                    displayName: 'Hamptons',
                    fullName: 'food_Decorating_Style_Hamptons',
                    name: 'food:Decorating:Style:Hamptons',
                    urlName: 'hamptons'
                },
                {
                    displayName: 'Heritage-listed',
                    fullName: 'food_Building_Building_style_Heritage_listed',
                    name: 'food:Building:Building style:Heritage-listed',
                    urlName: 'heritage-listed'
                },
                {
                    displayName: 'Terrace',
                    fullName: 'food_Building_Type_Terrace',
                    name: 'food:Building:Type:Terrace',
                    urlName: 'terrace'
                }
            ],
            url: '/darren-palmers-contemporary-classic-family-home-2993'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Darren Palmer house',
            contentImageCaption:
                'The interior balances contemporary lifestyle needs with Darren’s love of classic American homes, which he says “worked perfectly with the sweetness of the house”. Photo: Maree Homer | Styling: Ashley Pratt',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/17/2992/hero_C14B9601-9651.jpg',
            contentMonth: 2,
            contentSummary:
                'Celebrity interior designer and TV renovation judge Darren Palmer works his magic off the screen to create a contempoary-classic family home.',
            contentTitle: "Darren Palmer's dreamy yet durable Sydney home",
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-2992',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2016-02-17T04:41:51.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2992'],
            sortOrder: '46',
            source: 'Australian House and Garden',
            tags: [
                'food:Profiles:Celebrity',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Classic',
                'food:Location and setting:Setting:Inner-city',
                'food:Building:Building style:Heritage-listed',
                'food:Renovating:Materials:Stone',
                'food:Renovating:Materials:Linen',
                'food:Decorating:Style:Hamptons',
                'food:Building:Type:Terrace',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Stone',
                    fullName: 'food_Renovating_Materials_Stone',
                    name: 'food:Renovating:Materials:Stone',
                    urlName: 'stone'
                },
                {
                    displayName: 'Linen',
                    fullName: 'food_Renovating_Materials_Linen',
                    name: 'food:Renovating:Materials:Linen',
                    urlName: 'linen'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Inner-city',
                    fullName: 'food_Location_and_setting_Setting_Inner_city',
                    name: 'food:Location and setting:Setting:Inner-city',
                    urlName: 'inner-city'
                },
                {
                    displayName: 'Classic',
                    fullName: 'food_Decorating_Style_Classic',
                    name: 'food:Decorating:Style:Classic',
                    urlName: 'classic'
                },
                {
                    displayName: 'Hamptons',
                    fullName: 'food_Decorating_Style_Hamptons',
                    name: 'food:Decorating:Style:Hamptons',
                    urlName: 'hamptons'
                },
                {
                    displayName: 'Heritage-listed',
                    fullName: 'food_Building_Building_style_Heritage_listed',
                    name: 'food:Building:Building style:Heritage-listed',
                    urlName: 'heritage-listed'
                },
                {
                    displayName: 'Terrace',
                    fullName: 'food_Building_Type_Terrace',
                    name: 'food:Building:Type:Terrace',
                    urlName: 'terrace'
                }
            ],
            url: '/darren-palmers-dreamy-yet-durable-sydney-home-2992'
        },
        {
            contentHasVideo: true,
            contentImageAltText: 'Naomi Watts apartment',
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/04/2923/hero_watts-schreiber-manhattan-loft-06.jpg',
            contentMonth: 2,
            contentSummary: 'The family commissioned designers Ashe + Leandro to turn a cavernous loft into a warm family home.',
            contentTitle: 'Inside Naomi Watts and Liev Schreiber’s New York City Apartment',
            contentVideo: {
                id: '4737906228001'
            },
            contentYear: 2016,
            id: 'HOMES-2923',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2016-02-04T05:35:28.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2923'],
            sortOrder: '11',
            source: 'Homes To Love',
            tags: [
                'food:Building:Type:Apartment/unit',
                'food:Topic:Luxury home',
                'food:Profiles:Celebrity',
                'food:Decorating:Style:Luxe',
                'food:Building:Building style:Industrial',
                'food:Decorating:Style:Urban',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Luxe',
                    fullName: 'food_Decorating_Style_Luxe',
                    name: 'food:Decorating:Style:Luxe',
                    urlName: 'luxe'
                },
                {
                    displayName: 'Urban',
                    fullName: 'food_Decorating_Style_Urban',
                    name: 'food:Decorating:Style:Urban',
                    urlName: 'urban'
                },
                {
                    displayName: 'Industrial',
                    fullName: 'food_Building_Building_style_Industrial',
                    name: 'food:Building:Building style:Industrial',
                    urlName: 'industrial'
                },
                {
                    displayName: 'Apartment/unit',
                    fullName: 'food_Building_Type_Apartmentunit',
                    name: 'food:Building:Type:Apartment/unit',
                    urlName: 'apartment-unit'
                }
            ],
            url: '/inside-naomi-watt-and-liev-schreibers-new-york-city-apartment-2923'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Melissa Collison transforms 50s-style Sydney duplex',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/01/2906/Vaucluse-hero-(1).jpg',
            contentMonth: 2,
            contentSummary:
                'Knocking down this house and starting again seemed like the right thing to do, until the owners spent time living in the property.',
            contentTags: [
                'food:Building:Building style:Modernist',
                'food:Topic:Luxury home',
                'food:Topic:Feature home',
                'food:Decorating:Style:Contemporary',
                'food:Building:Building style:Contemporary building',
                'food:Decorating:Style:Luxe'
            ],
            contentTitle: 'Interior designer Melissa Collison transforms 50s-style Sydney duplex',
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-2906',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2016-02-01T06:15:39.00Z',
            pageSearchTags: [
                'Modernist',
                'modernist',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Feature home',
                'Feature-home',
                'feature-home',
                'feature home',
                'Contemporary',
                'contemporary',
                'Contemporary building',
                'Contemporary-building',
                'contemporary-building',
                'contemporary building',
                'Luxe',
                'luxe'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2906'],
            sortOrder: '7',
            source: 'Belle',
            tags: [
                'food:Building:Building style:Modernist',
                'food:Topic:Luxury home',
                'food:Topic:Feature home',
                'food:Decorating:Style:Contemporary',
                'food:Building:Building style:Contemporary building',
                'food:Decorating:Style:Luxe'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Feature home',
                    fullName: 'food_Topic_Feature_home',
                    name: 'food:Topic:Feature home',
                    urlName: 'feature-home'
                },
                {
                    displayName: 'Luxe',
                    fullName: 'food_Decorating_Style_Luxe',
                    name: 'food:Decorating:Style:Luxe',
                    urlName: 'luxe'
                },
                {
                    displayName: 'Contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary'
                },
                {
                    displayName: 'Modernist',
                    fullName: 'food_Building_Building_style_Modernist',
                    name: 'food:Building:Building style:Modernist',
                    urlName: 'modernist'
                },
                {
                    displayName: 'Contemporary building',
                    fullName: 'food_Building_Building_style_Contemporary_building',
                    name: 'food:Building:Building style:Contemporary building',
                    urlName: 'contemporary-building'
                }
            ],
            url: '/interior-designer-melissa-collison-transforms-50s-style-sydney-duplex-2906'
        },
        {
            contentHasVideo: false,
            contentImageAltText: "Harry Seidler's award winning modern marvel",
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/01/2905/JOADJA-HERO.jpg',
            contentMonth: 2,
            contentSummary: 'This sculptural masterpiece remains on the cutting edge of design.',
            contentTags: [
                'food:Building:Building style:Modernist',
                'food:Topic:Luxury home',
                'food:Building:Building style:Contemporary building',
                'food:Decorating:Style:Contemporary',
                'food:Homes navigation:Real Homes'
            ],
            contentTitle: "Harry Seidler's award winning modern marvel",
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-2905',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2016-02-01T05:51:21.00Z',
            pageSearchTags: [
                'Modernist',
                'modernist',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Contemporary building',
                'Contemporary-building',
                'contemporary-building',
                'contemporary building',
                'Contemporary',
                'contemporary',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2905'],
            sortOrder: '6',
            source: 'Belle',
            tags: [
                'food:Building:Building style:Modernist',
                'food:Topic:Luxury home',
                'food:Building:Building style:Contemporary building',
                'food:Decorating:Style:Contemporary',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary'
                },
                {
                    displayName: 'Modernist',
                    fullName: 'food_Building_Building_style_Modernist',
                    name: 'food:Building:Building style:Modernist',
                    urlName: 'modernist'
                },
                {
                    displayName: 'Contemporary building',
                    fullName: 'food_Building_Building_style_Contemporary_building',
                    name: 'food:Building:Building style:Contemporary building',
                    urlName: 'contemporary-building'
                }
            ],
            url: '/harry-seidlers-award-winning-modern-marvel-2905'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Modernist marvels with a contemporary flavour',
            contentImageCaption: "Harry Seidler's award-winning modernist marvel nestled in the Southern Highlands. Photo: Richard Powers",
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/01/2902/Modern-Marval-Hero.jpg',
            contentMonth: 2,
            contentSummary:
                '“Modernism released us from the constraints of everything that had gone before with a euphoric sense of freedom.” - Arthur Erickson.',
            contentTitle: 'Celebrating Australian design: amazing modernist homes',
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-2902',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2016-02-01T05:08:49.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2902'],
            sortOrder: '4',
            source: 'Belle',
            tags: [
                'food:Building:Building style:Modernist',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Contemporary',
                'food:Building:Building style:Contemporary building',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary'
                },
                {
                    displayName: 'Modernist',
                    fullName: 'food_Building_Building_style_Modernist',
                    name: 'food:Building:Building style:Modernist',
                    urlName: 'modernist'
                },
                {
                    displayName: 'Contemporary building',
                    fullName: 'food_Building_Building_style_Contemporary_building',
                    name: 'food:Building:Building style:Contemporary building',
                    urlName: 'contemporary-building'
                }
            ],
            url: '/three-modernist-marvels-with-a-contemporary-flavour-2902'
        },
        {
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/21/2640/hero_Nexus_Wollongara_0109FBELLE.jpg',
            contentMonth: 1,
            contentSummary: "A modern design aesthetic mixes with functional form in this revamp of a '70s country home.",
            contentTags: ['food:Location and setting:Setting:Rural', 'food:Topic:Luxury home', 'food:Homes navigation:Real Homes'],
            contentTitle: "Contemporary revamp for '70s farmhouse",
            contentVideo: '',
            contentYear: 2016,
            id: 'HOMES-2640',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2016-01-20T13:00:00.00Z',
            pageSearchTags: [
                'Rural',
                'rural',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2784', 'HOMES-2640'],
            sortOrder: '24',
            source: 'Belle',
            tags: ['food:Location and setting:Setting:Rural', 'food:Topic:Luxury home', 'food:Homes navigation:Real Homes'],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Rural',
                    fullName: 'food_Location_and_setting_Setting_Rural',
                    name: 'food:Location and setting:Setting:Rural',
                    urlName: 'rural'
                }
            ],
            url: '/contemporary-revamp-for-70s-farmhouse-2640'
        },
        {
            contentHasVideo: false,
            contentImageAltText: "interior designer's country retreat",
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/11/2636/HERO_SimonCarver_006-(1).jpg',
            contentMonth: 11,
            contentSummary:
                "An 1865 miner's cottage gets the loving touch of its interior designer owner. See how high-end details beautify an already charming home.",
            contentTags: [
                'food:Decorating:Style:Country',
                'food:Topic:Luxury home',
                'food:Location and setting:Setting:Rural',
                'food:Location and setting:Australia:Victoria:Daylesford and the Macedon Ranges',
                'food:Decorating:Style:Luxe',
                'food:Homes navigation:Real Homes'
            ],
            contentTitle: "An interior designer's chic country retreat",
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-2636',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2015-11-11T23:08:46.00Z',
            pageSearchTags: [
                'Country',
                'country',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Rural',
                'rural',
                'Daylesford and the Macedon Ranges',
                'Daylesford-and-the-Macedon-Ranges',
                'daylesford-and-the-macedon-ranges',
                'daylesford and the macedon ranges',
                'Luxe',
                'luxe',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-1580', 'HOMES-2497', 'HOMES-2636'],
            sortOrder: '54',
            source: 'Belle',
            tags: [
                'food:Decorating:Style:Country',
                'food:Topic:Luxury home',
                'food:Location and setting:Setting:Rural',
                'food:Location and setting:Australia:Victoria:Daylesford and the Macedon Ranges',
                'food:Decorating:Style:Luxe',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Luxe',
                    fullName: 'food_Decorating_Style_Luxe',
                    name: 'food:Decorating:Style:Luxe',
                    urlName: 'luxe'
                },
                {
                    displayName: 'Country',
                    fullName: 'food_Decorating_Style_Country',
                    name: 'food:Decorating:Style:Country',
                    urlName: 'country'
                },
                {
                    displayName: 'Daylesford and the Macedon Ranges',
                    fullName: 'food_Location_and_setting_Australia_Victoria_Daylesford_and_the_Macedon_Ranges',
                    name: 'food:Location and setting:Australia:Victoria:Daylesford and the Macedon Ranges',
                    urlName: 'daylesford-and-the-macedon-ranges'
                },
                {
                    displayName: 'Rural',
                    fullName: 'food_Location_and_setting_Setting_Rural',
                    name: 'food:Location and setting:Setting:Rural',
                    urlName: 'rural'
                }
            ],
            url: '/interior-designer-capitalises-on-cottage-charm-2636'
        },
        {
            contentHasVideo: false,
            contentImageAltText: "Daylesford miner's cottage",
            contentImageCaption:
                '“Finding comfort, the right pieces, and the right scale and size for a room is important,” says homeowner and interior designer Simon Carver of [Carver Design](http://www.carverdesign.com.au/?utm_campaign=supplier/|target="_blank"). Photo: Derek Swalwell | Styling: Beck Simon',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/11/2622/hero_SimonCarver_005.jpg',
            contentMonth: 11,
            contentSummary: 'An interior designer’s historic retreat mixes cottage charm and high-end details with finesse.',
            contentTitle: "Dramatic makeover for Daylesford miner's cottage",
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-2622',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2015-11-10T23:19:00.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-1393', 'HOMES-2500', 'HOMES-2622'],
            sortOrder: '65',
            source: 'Belle',
            tags: [
                'food:Decorating:Style:Country',
                'food:Topic:Luxury home',
                'food:Location and setting:Setting:Rural',
                'food:Location and setting:Australia:Victoria:Daylesford and the Macedon Ranges',
                'food:Decorating:Style:Luxe',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Luxe',
                    fullName: 'food_Decorating_Style_Luxe',
                    name: 'food:Decorating:Style:Luxe',
                    urlName: 'luxe'
                },
                {
                    displayName: 'Country',
                    fullName: 'food_Decorating_Style_Country',
                    name: 'food:Decorating:Style:Country',
                    urlName: 'country'
                },
                {
                    displayName: 'Daylesford and the Macedon Ranges',
                    fullName: 'food_Location_and_setting_Australia_Victoria_Daylesford_and_the_Macedon_Ranges',
                    name: 'food:Location and setting:Australia:Victoria:Daylesford and the Macedon Ranges',
                    urlName: 'daylesford-and-the-macedon-ranges'
                },
                {
                    displayName: 'Rural',
                    fullName: 'food_Location_and_setting_Setting_Rural',
                    name: 'food:Location and setting:Setting:Rural',
                    urlName: 'rural'
                }
            ],
            url: '/city-style-in-a-country-cottage-2622'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'luxury dining room',
            contentImageCaption:
                '“The longer the design period the more special things can be,” says architect Matt Gibson who spent five years overseeing the overhaul of this Victorian house. Photo: Shannon McGrath',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2015/11/18/2572/2MG_AlbertPark_-smg_178942.jpg',
            contentMonth: 11,
            contentSummary:
                'Putting a modern spin on a 19th-century house is not for the faint-hearted. Architect Matt Gibson stepped up to the challenge.',
            contentTitle: 'Something old, something new: one heritage home gets a beautifully modern revamp',
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-2572',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2015-11-06T01:43:53.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-1393', 'HOMES-2500', 'HOMES-2572'],
            sortOrder: '35',
            source: 'Belle',
            tags: [
                'food:Building:Building style:Victorian',
                'food:Topic:Luxury home',
                'tv:tv_subject:Renovation',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Victorian',
                    fullName: 'food_Building_Building_style_Victorian',
                    name: 'food:Building:Building style:Victorian',
                    urlName: 'victorian'
                }
            ],
            url: '/bold-new-vision-for-a-heritage-home-2572'
        },
        {
            contentHasVideo: false,
            contentImageCaption:
                'To modernise a double-fronted Victorian house, architect and interior designer [Matt Gibson](http://mattgibson.com.au/?utm_campaign=supplier/|target="_blank") created a bold\r\nextension that marries past and present.',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/25/2825/James-Hero.jpg',
            contentMonth: 11,
            contentSummary:
                'Putting a modern spin on a 19th-century house is not for the faint-hearted, but architect Matt Gibson was up for the challenge.',
            contentTitle: 'Bold revision for worn-out Victorian',
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-2549',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2015-11-05T01:47:00.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-1393', 'HOMES-2500', 'HOMES-2549'],
            sortOrder: '25',
            source: 'Belle',
            tags: [
                'food:Renovating:Renovation type:Extension',
                'food:Topic:Luxury home',
                'food:Building:Building style:Victorian',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Extension',
                    fullName: 'food_Renovating_Renovation_type_Extension',
                    name: 'food:Renovating:Renovation type:Extension',
                    urlName: 'extension'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Victorian',
                    fullName: 'food_Building_Building_style_Victorian',
                    name: 'food:Building:Building style:Victorian',
                    urlName: 'victorian'
                }
            ],
            url: '/bold-revision-for-worn-out-victorian-2549'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Art deco',
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2015/12/18/2424/HERO1FMDHouse_FINAL0026_v0.5-(1).jpg',
            contentMonth: 10,
            contentSummary:
                'Taking this pristine 1930s house into the 21st century required a light touch but this stately home is now fit for family living.',
            contentTags: [
                'food:Building:Building style:Art Deco',
                'food:Topic:Luxury home',
                'food:Location and setting:Australia:Victoria:Melbourne',
                'food:Homes navigation:Real Homes'
            ],
            contentTitle: 'Respectful revamp for Melbourne grande dame',
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-2424',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2015-10-26T23:39:41.00Z',
            pageSearchTags: [
                'Art Deco',
                'Art-Deco',
                'art-deco',
                'art deco',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Melbourne',
                'melbourne',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-1580', 'HOMES-2215', 'HOMES-2424'],
            sortOrder: '82',
            source: 'Belle',
            tags: [
                'food:Building:Building style:Art Deco',
                'food:Topic:Luxury home',
                'food:Location and setting:Australia:Victoria:Melbourne',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Art Deco',
                    fullName: 'food_Building_Building_style_Art_Deco',
                    name: 'food:Building:Building style:Art Deco',
                    urlName: 'art-deco'
                }
            ],
            url: '/respectful-revamp-for-melbourne-grande-dame-2424'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'coastal style Sydney',
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2015/10/20/2380/GALLERY-HERO_150204---BELLE---AVALON-HOME-9172_v0.4.jpg',
            contentMonth: 10,
            contentSummary:
                'With a pastel palette, natural materials and an easy connection with its beachfront locale, this home is a sanctuary for its owners.',
            contentTags: [
                'food:Decorating:Style:Coastal/beach',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Hamptons',
                'food:Location and setting:Australia:New South Wales:Sydney:Sydney northern beaches',
                'food:Homes navigation:Real Homes'
            ],
            contentTitle: 'Luxurious beach house renovation',
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-2380',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2015-10-22T00:32:00.00Z',
            pageSearchTags: [
                'Coastal/beach',
                'coastal/beach',
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Hamptons',
                'hamptons',
                'Sydney northern beaches',
                'Sydney-northern-beaches',
                'sydney-northern-beaches',
                'sydney northern beaches',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-1580', 'HOMES-2215', 'HOMES-2380'],
            sortOrder: '67',
            source: 'Belle',
            tags: [
                'food:Decorating:Style:Coastal/beach',
                'food:Topic:Luxury home',
                'food:Decorating:Style:Hamptons',
                'food:Location and setting:Australia:New South Wales:Sydney:Sydney northern beaches',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Hamptons',
                    fullName: 'food_Decorating_Style_Hamptons',
                    name: 'food:Decorating:Style:Hamptons',
                    urlName: 'hamptons'
                },
                {
                    displayName: 'Coastal/beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach'
                },
                {
                    displayName: 'Sydney northern beaches',
                    fullName: 'food_Location_and_setting_Australia_New_South_Wales_Sydney_Sydney_northern_beaches',
                    name: 'food:Location and setting:Australia:New South Wales:Sydney:Sydney northern beaches',
                    urlName: 'sydney-northern-beaches'
                }
            ],
            url: '/gallery-luxurious-beach-house-renovation-2380'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Modern retro living room',
            contentImageCaption:
                '“My brief for the living room was a confusing mix between New York loft style and classic with a contemporary edge,” say Vivienne. “I didn’t even know what that meant, but our interior designer Sally was able to work with it and quickly understood what we liked.”',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2015/07/30/1672/ARTICLE-HERO_ElwoodHouse-02.jpg',
            contentMonth: 10,
            contentSummary:
                'A couple of expats return to Melbourne, buy a house, and engage interior designer Sally Caroline Klopper to create a high-end hotel vibe.',
            contentTitle: 'Vivienne and Max’s luxe hotel-inspired home',
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-1672',
            nodeTypeAlias: 'HomesArticle',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            pageDateCreated: '2015-10-22T00:00:00.00Z',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-1393', 'HOMES-2207', 'HOMES-1672'],
            sortOrder: '103',
            source: 'real living',
            tags: [
                'food:Location and setting:Australia:Victoria:Melbourne:Melbourne south',
                'food:Topic:Luxury home',
                'food:Building:Type:House',
                'tv:tv_subject:Renovation',
                'food:Decorating:Style:Luxe',
                'food:Decorating:Style:Contemporary',
                'food:Building:Type:Terrace',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Luxe',
                    fullName: 'food_Decorating_Style_Luxe',
                    name: 'food:Decorating:Style:Luxe',
                    urlName: 'luxe'
                },
                {
                    displayName: 'Contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary'
                },
                {
                    displayName: 'House',
                    fullName: 'food_Building_Type_House',
                    name: 'food:Building:Type:House',
                    urlName: 'house'
                },
                {
                    displayName: 'Terrace',
                    fullName: 'food_Building_Type_Terrace',
                    name: 'food:Building:Type:Terrace',
                    urlName: 'terrace'
                },
                {
                    displayName: 'Melbourne south',
                    fullName: 'food_Location_and_setting_Australia_Victoria_Melbourne_Melbourne_south',
                    name: 'food:Location and setting:Australia:Victoria:Melbourne:Melbourne south',
                    urlName: 'melbourne-south'
                }
            ],
            url: '/vivienne-and-maxs-luxe-hotel-inspired-home-1672'
        },
        {
            contentHasVideo: false,
            contentImageAltText: 'Luxury brick and steel open plan living',
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2015/11/26/2453/HERO_be0614_Home_Inglis_02R1.jpg',
            contentMonth: 10,
            contentSummary:
                'A sculptural facade, open-plan design and restrained palette of materials add up to a smart, spacious feel for this inner-city Melbourne home.',
            contentTags: [
                'food:Topic:Luxury home',
                'food:Location and setting:Australia:Victoria:Melbourne:Melbourne inner city',
                'food:Location and setting:Setting:Inner-city',
                'food:Renovating:Materials:Brick',
                'food:Homes navigation:Real Homes'
            ],
            contentTitle: 'A luxury brick and steel architectural feat',
            contentVideo: '',
            contentYear: 2015,
            id: 'HOMES-2453',
            nodeTypeAlias: 'Gallery',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            pageDateCreated: '2015-10-20T13:00:00.00Z',
            pageSearchTags: [
                'Luxury home',
                'Luxury-home',
                'luxury-home',
                'luxury home',
                'Melbourne inner city',
                'Melbourne-inner-city',
                'melbourne-inner-city',
                'melbourne inner city',
                'Inner-city',
                'inner-city',
                'Brick',
                'brick',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-1580', 'HOMES-2215', 'HOMES-2453'],
            sortOrder: '93',
            source: 'Belle',
            tags: [
                'food:Topic:Luxury home',
                'food:Location and setting:Australia:Victoria:Melbourne:Melbourne inner city',
                'food:Location and setting:Setting:Inner-city',
                'food:Renovating:Materials:Brick',
                'food:Homes navigation:Real Homes'
            ],
            tagsDetails: [
                {
                    displayName: 'Luxury home',
                    fullName: 'food_Topic_Luxury_home',
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home'
                },
                {
                    displayName: 'Brick',
                    fullName: 'food_Renovating_Materials_Brick',
                    name: 'food:Renovating:Materials:Brick',
                    urlName: 'brick'
                },
                {
                    displayName: 'Real Homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes'
                },
                {
                    displayName: 'Inner-city',
                    fullName: 'food_Location_and_setting_Setting_Inner_city',
                    name: 'food:Location and setting:Setting:Inner-city',
                    urlName: 'inner-city'
                },
                {
                    displayName: 'Melbourne inner city',
                    fullName: 'food_Location_and_setting_Australia_Victoria_Melbourne_Melbourne_inner_city',
                    name: 'food:Location and setting:Australia:Victoria:Melbourne:Melbourne inner city',
                    urlName: 'melbourne-inner-city'
                }
            ],
            url: '/a-luxury-brick-and-steel-architectural-feat-2453'
        }
    ]
};
