export default {
    totalCount: 55,
    data: [
        {
            id: 'HOMES-3210',
            url: '/template-htl-poll-3210',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-3210'],
            tags: ['food:Topic:Luxury home', 'food:Homes navigation:Bedroom'],
            source: 'homes+',
            sortOrder: '2',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Luxury home',
                    urlName: 'luxury-home',
                    fullName: 'food_Topic_Luxury_home',
                    displayName: 'Luxury home'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                }
            ],
            contentMonth: 12,
            contentTitle: 'Template HTL Poll',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentHasVideo: false,
            contentImageUrl:
                'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2017/10/16/1508131915542_14786528301071436507825996140718-REALLIVING-RENATA99of286v0.4.jpg',
            pageDateCreated: '2016-12-08T05:04:01.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-2828',
            url: '/cool-makeover-for-teen-bedroom-2828',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3206', 'HOMES-2828'],
            tags: [
                "food:Room:Bedroom:Girl's bedroom",
                'food:Room:Bedroom:Teen bedroom',
                'food:Topic:Decorating 101',
                'food:Topic:Before and after',
                'food:Decorating:Style:Contemporary',
                'food:Decorating:Colour:Black and white',
                'food:Homes navigation:Bedroom'
            ],
            source: 'homes+',
            sortOrder: '1',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Decorating 101',
                    urlName: 'decorating-101',
                    fullName: 'food_Topic_Decorating_101',
                    displayName: 'Decorating 101'
                },
                {
                    name: 'food:Topic:Before and after',
                    urlName: 'before-and-after',
                    fullName: 'food_Topic_Before_and_after',
                    displayName: 'Before and after'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Decorating:Colour:Black and white',
                    urlName: 'black-and-white',
                    fullName: 'food_Decorating_Colour_Black_and_white',
                    displayName: 'Black and white'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                },
                {
                    name: 'food:Room:Bedroom:Teen bedroom',
                    urlName: 'teen-bedroom',
                    fullName: 'food_Room_Bedroom_Teen_bedroom',
                    displayName: 'Teen bedroom'
                },
                {
                    name: "food:Room:Bedroom:Girl's bedroom",
                    urlName: 'girls-bedroom',
                    fullName: 'food_Room_Bedroom_Girls_bedroom',
                    displayName: "Girl's bedroom"
                }
            ],
            contentMonth: 12,
            contentTitle: 'Teen bedroom grows up TEST',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: "A cool, modern makeover tranforms 13-year old Emma's bedroom into a stylish retreat she can be proud of.",
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/13/2828/HEROOIMG_297-copy.jpg',
            pageDateCreated: '2016-12-01T13:00:00.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageCaption:
                'Emma chose the decor for her new room, opting for bargains she can mix and match to suit her budget and evolving style. She lives with her mum, dad, sister and Casper, their nine-year-old bichon frise. '
        },
        {
            id: 'HOMES-3052',
            url: '/before-and-after-stylish-black-kitchen-transformation-test-3052',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3049', 'HOMES-3052'],
            tags: [
                'food:Decorating:Colour:Black',
                'food:Topic:DIY Projects',
                'food:Room:Kitchen:Kitchens',
                'tv:tv_subject:Renovation',
                'food:Renovating:Renovation type:Remodel',
                'food:Homes navigation:Kitchen'
            ],
            source: 'homes+',
            sortOrder: '2',
            contentYear: 2016,
            sponsorName: 'Myer Eat Live',
            tagsDetails: [
                {
                    name: 'food:Renovating:Renovation type:Remodel',
                    urlName: 'remodel',
                    fullName: 'food_Renovating_Renovation_type_Remodel',
                    displayName: 'Remodel'
                },
                {
                    name: 'food:Topic:DIY Projects',
                    urlName: 'diy-projects',
                    fullName: 'food_Topic_DIY_Projects',
                    displayName: 'DIY Projects'
                },
                {
                    name: 'food:Homes navigation:Kitchen',
                    urlName: 'kitchen',
                    fullName: 'food_Homes_navigation_Kitchen',
                    displayName: 'Kitchen'
                },
                {
                    name: 'food:Decorating:Colour:Black',
                    urlName: 'black',
                    fullName: 'food_Decorating_Colour_Black',
                    displayName: 'Black'
                },
                {
                    name: 'food:Room:Kitchen:Kitchens',
                    urlName: 'kitchens',
                    fullName: 'food_Room_Kitchen_Kitchens',
                    displayName: 'Kitchens'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Before & after: Stylish black kitchen transformation',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'A tired old kitchen is reinvented with a designer look thanks to this DIY-savvy duo.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/03/01/3052/hero_Sally-Adam---after-(2).jpg',
            pageDateCreated: '2016-02-29T23:27:51.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'black kitchen designs',
            contentImageCaption: 'The couple’s biggest challenge was making the most of the small space available to them.'
        },
        {
            id: 'HOMES-3048',
            url: '/diy-kitchen-remodel-on-a-budget-test-1-3048',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-3049', 'HOMES-3048'],
            tags: ['food:Topic:Tips and advice', 'food:Room:Kitchen:Kitchen island', 'food:Homes navigation:DIY'],
            source: 'homes+',
            sortOrder: '1',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'food:Room:Kitchen:Kitchen island',
                    urlName: 'kitchen-island',
                    fullName: 'food_Room_Kitchen_Kitchen_island',
                    displayName: 'Kitchen island'
                }
            ],
            contentMonth: 2,
            contentTitle: 'DIY kitchen remodel on a budget - Janice Test',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'A young couple on a strict budget gave their dingy kitchen a fresh makeover with an industrial-style edge.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/03/01/3048/hero_Jack-Ali---after-(2).jpg',
            pageDateCreated: '2016-02-29T22:27:50.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'DIY kitchen remodel on a budget',
            contentImageCaption:
                'Jack designed and built the open, steel- framed shelving and used plain plywood for the panelling around the fridge.'
        },
        {
            id: 'HOMES-3039',
            url: '/weekend-project-diy-timber-plant-holder-3039',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-3039'],
            tags: [
                '"food:Products, Furniture and Fittings:Furniture and fittings by room:Outdoor furniture:Planters and pots"',
                'food:Topic:DIY Projects',
                'food:Garden/Outdoor:Plants:Potted plants',
                'food:Homes navigation:Outdoor'
            ],
            source: 'homes+',
            sortOrder: '72',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:DIY Projects',
                    urlName: 'diy-projects',
                    fullName: 'food_Topic_DIY_Projects',
                    displayName: 'DIY Projects'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                },
                {
                    name: 'food:Garden/Outdoor:Plants:Potted plants',
                    urlName: 'potted-plants',
                    fullName: 'food_GardenOutdoor_Plants_Potted_plants',
                    displayName: 'Potted plants'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Weekend project: DIY Timber plant holder',
            contentVideo: {
                id: '4774850618001',
                name: 'SCOTT CAM DIY Potted plant stand - homes',
                type: 'VideoItem',
                properties: {
                    videoConfiguration: {
                        status: '',
                        videoId: '4774850618001',
                        statusCode: 200,
                        brightcoveId: '4774850618001',
                        thumbnailUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4774868801001_4774850618001-th.jpg?pubId=761709621001',
                        videoStillUrl:
                            'http://brightcove04.o.brightcove.com/761709621001/761709621001_4774868800001_4774850618001-vs.jpg?pubId=761709621001'
                    }
                }
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Scotty Cam shows you how to easily create a fun plant shelf for your kitchen windowsill – perfect for inside or out.',
            contentHasVideo: true,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/26/3039/HERO-2011.jpg',
            pageDateCreated: '2016-02-26T05:26:30.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'timber plant holder'
        },
        {
            id: 'HOMES-3032',
            url: '/country-cottage-with-character-3032',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-3032'],
            tags: [
                'food:Decorating:Style:Contemporary',
                "food:Topic:Readers' home",
                'food:Decorating:Style:Country',
                'food:Building:Building style:Cottage',
                'food:Homes navigation:Real Homes'
            ],
            source: 'homes+',
            sortOrder: '34',
            contentTags: [
                'food:Decorating:Style:Contemporary',
                "food:Topic:Readers' home",
                'food:Decorating:Style:Country',
                'food:Building:Building style:Cottage',
                'food:Homes navigation:Real Homes'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: "food:Topic:Readers' home",
                    urlName: 'readers-home',
                    fullName: 'food_Topic_Readers_home',
                    displayName: "Readers' home"
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                },
                {
                    name: 'food:Decorating:Style:Country',
                    urlName: 'country',
                    fullName: 'food_Decorating_Style_Country',
                    displayName: 'Country'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                },
                {
                    name: 'food:Building:Building style:Cottage',
                    urlName: 'cottage',
                    fullName: 'food_Building_Building_style_Cottage',
                    displayName: 'Cottage'
                }
            ],
            contentMonth: 2,
            contentTitle: 'This character filled country cottage makes the perfect family home',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'This Queensland couple found a cute cottage that’s perfect for their young family.',
            pageSearchTags: [
                'Contemporary',
                'contemporary',
                "Readers' home",
                "Readers'-home",
                "readers'-home",
                "readers' home",
                'Country',
                'country',
                'Cottage',
                'cottage',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/25/3032/Cottage-HERO.jpg',
            pageDateCreated: '2016-02-25T02:46:59.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Character filled country cottage '
        },
        {
            id: 'HOMES-3031',
            url: '/environmentally-friendly-contemporary-family-home-3031',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-3031'],
            tags: [
                'food:Decorating:Style:Contemporary',
                "food:Topic:Readers' home",
                'food:Renovating:Sustainable living:Sustainable materials',
                'food:Renovating:Sustainable living:Green building',
                'food:Homes navigation:Real Homes'
            ],
            source: 'homes+',
            sortOrder: '33',
            contentTags: [
                'food:Decorating:Style:Contemporary',
                "food:Topic:Readers' home",
                'food:Renovating:Sustainable living:Sustainable materials',
                'food:Renovating:Sustainable living:Green building',
                'food:Homes navigation:Real Homes'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: "food:Topic:Readers' home",
                    urlName: 'readers-home',
                    fullName: 'food_Topic_Readers_home',
                    displayName: "Readers' home"
                },
                {
                    name: 'food:Renovating:Sustainable living:Sustainable materials',
                    urlName: 'sustainable-materials',
                    fullName: 'food_Renovating_Sustainable_living_Sustainable_materials',
                    displayName: 'Sustainable materials'
                },
                {
                    name: 'food:Renovating:Sustainable living:Green building',
                    urlName: 'green-building',
                    fullName: 'food_Renovating_Sustainable_living_Green_building',
                    displayName: 'Green building'
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                },
                {
                    name: 'food:Decorating:Style:Contemporary',
                    urlName: 'contemporary',
                    fullName: 'food_Decorating_Style_Contemporary',
                    displayName: 'Contemporary'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Environmentally friendly contemporary family home',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Buying a “weekender” marked the beginning of a lifestyle change for Brigette and her young family.',
            pageSearchTags: [
                'Contemporary',
                'contemporary',
                "Readers' home",
                "Readers'-home",
                "readers'-home",
                "readers' home",
                'Sustainable materials',
                'Sustainable-materials',
                'sustainable-materials',
                'sustainable materials',
                'Green building',
                'Green-building',
                'green-building',
                'green building',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/24/3031/Home-HERO.jpg',
            pageDateCreated: '2016-02-24T05:27:17.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Environmentally friendly contemporary family home'
        },
        {
            id: 'HOMES-3030',
            url: '/weekend-project-diy-wooden-clock-3030',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-3030'],
            tags: [
                '"food:Products, Furniture and Fittings:Accessories and decor:Clocks"',
                'food:Topic:DIY Projects',
                'food:Renovating:Materials:Timber',
                'food:DIY and craft:DIY projects:DIY gift projects',
                'food:DIY and craft:DIY projects:Home accessories projects',
                'food:Homes navigation:DIY'
            ],
            source: 'homes+',
            sortOrder: '68',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:DIY and craft:DIY projects:DIY gift projects',
                    urlName: 'diy-gift-projects',
                    fullName: 'food_DIY_and_craft_DIY_projects_DIY_gift_projects',
                    displayName: 'DIY gift projects'
                },
                {
                    name: 'food:DIY and craft:DIY projects:Home accessories projects',
                    urlName: 'home-accessories-projects',
                    fullName: 'food_DIY_and_craft_DIY_projects_Home_accessories_projects',
                    displayName: 'Home accessories projects'
                },
                {
                    name: 'food:Renovating:Materials:Timber',
                    urlName: 'timber',
                    fullName: 'food_Renovating_Materials_Timber',
                    displayName: 'Timber'
                },
                {
                    name: 'food:Topic:DIY Projects',
                    urlName: 'diy-projects',
                    fullName: 'food_Topic_DIY_Projects',
                    displayName: 'DIY Projects'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Weekend project: DIY wooden clock',
            contentVideo: {
                id: '4739698351001'
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Style up your work space with this make-it-yourself timekeeper.',
            contentHasVideo: true,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/24/3030/hero_151019H-DIY-7485.jpg',
            pageDateCreated: '2016-02-24T04:18:20.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'clock DIY projects'
        },
        {
            id: 'HOMES-2984',
            url: '/homes-march-2016-giveaway-2984',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2984'],
            tags: ['food:Topic:Decorating 101', 'food:Homes navigation:DIY'],
            source: 'homes+',
            sortOrder: '43',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Decorating 101',
                    urlName: 'decorating-101',
                    fullName: 'food_Topic_Decorating_101',
                    displayName: 'Decorating 101'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                }
            ],
            contentMonth: 2,
            contentTitle: 'homes+ March 2016 giveaway',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Have fun and deck out your home, kitchen and even your pets with these fabulous prizes.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/18/2984/homes-mar-16.jpg',
            pageDateCreated: '2016-02-15T22:13:16.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle']
        },
        {
            id: 'HOMES-2956',
            url: '/shop-the-look-living-room-makeover-2956',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2956'],
            tags: [
                'food:Room:Living:Living rooms',
                'food:Topic:Before and after',
                'food:Decorating:Style:Coastal/beach',
                'food:Room:Living:Family living room',
                'food:Room:Living:Open plan living room',
                'food:Decorating:Colour:Blue:Navy blue',
                'food:Decorating:Colour:White',
                'food:Renovating:Renovation type:Remodel',
                'food:Decorating:Style:Hamptons',
                'food:Homes navigation:Living'
            ],
            source: 'homes+',
            sortOrder: '20',
            contentTags: [
                'food:Room:Living:Living rooms',
                'food:Topic:Before and after',
                'food:Decorating:Style:Coastal/beach',
                'food:Room:Living:Family living room',
                'food:Room:Living:Open plan living room',
                'food:Decorating:Colour:Blue:Navy blue',
                'food:Decorating:Colour:White',
                'food:Renovating:Renovation type:Remodel',
                'food:Decorating:Style:Hamptons',
                'food:Homes navigation:Living'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Before and after',
                    urlName: 'before-and-after',
                    fullName: 'food_Topic_Before_and_after',
                    displayName: 'Before and after'
                },
                {
                    name: 'food:Renovating:Renovation type:Remodel',
                    urlName: 'remodel',
                    fullName: 'food_Renovating_Renovation_type_Remodel',
                    displayName: 'Remodel'
                },
                {
                    name: 'food:Homes navigation:Living',
                    urlName: 'living',
                    fullName: 'food_Homes_navigation_Living',
                    displayName: 'Living'
                },
                {
                    name: 'food:Decorating:Colour:Blue:Navy blue',
                    urlName: 'navy-blue',
                    fullName: 'food_Decorating_Colour_Blue_Navy_blue',
                    displayName: 'Navy blue'
                },
                {
                    name: 'food:Decorating:Colour:White',
                    urlName: 'white',
                    fullName: 'food_Decorating_Colour_White',
                    displayName: 'White'
                },
                {
                    name: 'food:Decorating:Style:Hamptons',
                    urlName: 'hamptons',
                    fullName: 'food_Decorating_Style_Hamptons',
                    displayName: 'Hamptons'
                },
                {
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    displayName: 'Coastal/beach'
                },
                {
                    name: 'food:Room:Living:Living rooms',
                    urlName: 'living-rooms',
                    fullName: 'food_Room_Living_Living_rooms',
                    displayName: 'Living rooms'
                },
                {
                    name: 'food:Room:Living:Open plan living room',
                    urlName: 'open-plan-living-room',
                    fullName: 'food_Room_Living_Open_plan_living_room',
                    displayName: 'Open plan living room'
                },
                {
                    name: 'food:Room:Living:Family living room',
                    urlName: 'family-living-room',
                    fullName: 'food_Room_Living_Family_living_room',
                    displayName: 'Family living room'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Shop the look: living room makeover',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Super stylist Jackie helps a *homes+* reader revitalise a family living room with a budget of $1000.',
            pageSearchTags: [
                'Living rooms',
                'Living-rooms',
                'living-rooms',
                'living rooms',
                'Before and after',
                'Before-and-after',
                'before-and-after',
                'before and after',
                'Coastal/beach',
                'coastal/beach',
                'Family living room',
                'Family-living-room',
                'family-living-room',
                'family living room',
                'Open plan living room',
                'Open-plan-living-room',
                'open-plan-living-room',
                'open plan living room',
                'Navy blue',
                'Navy-blue',
                'navy-blue',
                'navy blue',
                'White',
                'white',
                'Remodel',
                'remodel',
                'Hamptons',
                'hamptons',
                'Living',
                'living'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/11/2956/150918H-style-5332.jpg',
            pageDateCreated: '2016-02-10T23:07:06.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'living room makeovers on a budget'
        },
        {
            id: 'HOMES-2954',
            url: '/weekend-project-diy-wire-baskets-2954',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2954'],
            tags: [
                'food:Products, Furniture and Fittings:Accessories and decor:Baskets and boxes',
                'food:Topic:DIY Projects',
                'food:DIY and craft:DIY projects:Home accessories projects',
                'food:Products, Furniture and Fittings:Furniture and fittings by room:Office furniture:Filing and storage',
                'food:Room:Other rooms:Study/Home office',
                'food:Room:Other rooms:Study nook',
                'food:Homes navigation:Living'
            ],
            source: 'homes+',
            sortOrder: '26',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:DIY and craft:DIY projects:Home accessories projects',
                    urlName: 'home-accessories-projects',
                    fullName: 'food_DIY_and_craft_DIY_projects_Home_accessories_projects',
                    displayName: 'Home accessories projects'
                },
                {
                    name: 'food:Homes navigation:Living',
                    urlName: 'living',
                    fullName: 'food_Homes_navigation_Living',
                    displayName: 'Living'
                },
                {
                    name: 'food:Topic:DIY Projects',
                    urlName: 'diy-projects',
                    fullName: 'food_Topic_DIY_Projects',
                    displayName: 'DIY Projects'
                },
                {
                    name: 'food:Products, Furniture and Fittings:Accessories and decor:Baskets and boxes',
                    urlName: 'baskets-and-boxes',
                    fullName: 'food_Products_Furniture_and_Fittings_Accessories_and_decor_Baskets_and_boxes',
                    displayName: 'Baskets and boxes'
                },
                {
                    name: 'food:Products, Furniture and Fittings:Furniture and fittings by room:Office furniture:Filing and storage',
                    urlName: 'filing-and-storage',
                    fullName: 'food_Products_Furniture_and_Fittings_Furniture_and_fittings_by_room_Office_furniture_Filing_and_storage',
                    displayName: 'Filing and storage'
                },
                {
                    name: 'food:Room:Other rooms:Study nook',
                    urlName: 'study-nook',
                    fullName: 'food_Room_Other_rooms_Study_nook',
                    displayName: 'Study nook'
                },
                {
                    name: 'food:Room:Other rooms:Study/Home office',
                    urlName: 'study-home-office',
                    fullName: 'food_Room_Other_rooms_StudyHome_office',
                    displayName: 'Study/Home office'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Weekend project: DIY wire baskets',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Custom shapes and sizes mean you can make these wire baskets to fit any purpose or space.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/10/2954/hero_151110H-DIY-3204.jpg',
            pageDateCreated: '2016-02-10T04:57:08.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'DIY home office storage',
            contentImageCaption: 'Make customised baskets to fit specific books, folders, papers and files. Photo: James Henry'
        },
        {
            id: 'HOMES-2939',
            url: '/weekend-project-diy-rope-pendant-2939',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2939'],
            tags: [
                'food:DIY and craft:DIY projects:Lighting projects',
                'food:Topic:DIY Projects',
                'food:Products, Furniture and Fittings:Lighting:Pendant light',
                'food:DIY and craft:DIY projects:DIY gift projects',
                'food:Room:Bedroom:Bedrooms',
                'food:Products, Furniture and Fittings:Lighting:Sculptural lighting',
                'food:Renovating:Materials:Timber',
                'food:Homes navigation:Bedroom'
            ],
            source: 'homes+',
            sortOrder: '18',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:DIY and craft:DIY projects:DIY gift projects',
                    urlName: 'diy-gift-projects',
                    fullName: 'food_DIY_and_craft_DIY_projects_DIY_gift_projects',
                    displayName: 'DIY gift projects'
                },
                {
                    name: 'food:DIY and craft:DIY projects:Lighting projects',
                    urlName: 'lighting-projects',
                    fullName: 'food_DIY_and_craft_DIY_projects_Lighting_projects',
                    displayName: 'Lighting projects'
                },
                {
                    name: 'food:Renovating:Materials:Timber',
                    urlName: 'timber',
                    fullName: 'food_Renovating_Materials_Timber',
                    displayName: 'Timber'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Topic:DIY Projects',
                    urlName: 'diy-projects',
                    fullName: 'food_Topic_DIY_Projects',
                    displayName: 'DIY Projects'
                },
                {
                    name: 'food:Products, Furniture and Fittings:Lighting:Sculptural lighting',
                    urlName: 'sculptural-lighting',
                    fullName: 'food_Products_Furniture_and_Fittings_Lighting_Sculptural_lighting',
                    displayName: 'Sculptural lighting'
                },
                {
                    name: 'food:Products, Furniture and Fittings:Lighting:Pendant light',
                    urlName: 'pendant-light',
                    fullName: 'food_Products_Furniture_and_Fittings_Lighting_Pendant_light',
                    displayName: 'Pendant light'
                },
                {
                    name: 'food:Room:Bedroom:Bedrooms',
                    urlName: 'bedrooms',
                    fullName: 'food_Room_Bedroom_Bedrooms',
                    displayName: 'Bedrooms'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Weekend project: DIY rope pendant',
            contentVideo: {
                id: '4739698863001'
            },
            nodeTypeAlias: 'HomesArticle',
            contentSummary:
                'It may look tricky, but it couldn’t be simpler to take an everyday pendant light from ordinary to an original style statement. All you need is rope!',
            contentHasVideo: true,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/09/2939/hero_151020H-DIY-7746.jpg',
            pageDateCreated: '2016-02-08T23:14:01.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'rope pendant light DIY'
        },
        {
            id: 'HOMES-2929',
            url: '/weekend-project-diy-clay-planters-2929',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2929'],
            tags: ['food:DIY and craft:DIY projects', 'food:Topic:DIY Projects', 'food:DIY and craft:How-to:Decorating', 'food:Homes navigation:DIY'],
            source: 'homes+',
            sortOrder: '13',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:DIY and craft:How-to:Decorating',
                    urlName: 'decorating',
                    fullName: 'food_DIY_and_craft_How_to_Decorating',
                    displayName: 'Decorating'
                },
                {
                    name: 'food:Topic:DIY Projects',
                    urlName: 'diy-projects',
                    fullName: 'food_Topic_DIY_Projects',
                    displayName: 'DIY Projects'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Weekend project: DIY clay planters',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Spruce up your home with these handmade clay planters involving minimum effort for maximum impact.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/05/2929/Clay-Planters.jpg',
            pageDateCreated: '2016-02-05T03:20:22.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'Weekend project: DIY clay planters',
            contentImageCaption:
                'Perfect for potted succulents, these planters look great grouped together. Photo: James Henry | Styling: Fiona Michelon & Corina Koch '
        },
        {
            id: 'HOMES-2918',
            url: '/empty-nesters-embrace-coastal-style-2918',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2892', 'HOMES-2918'],
            tags: ['food:Decorating:Style:Coastal/beach', 'food:Topic:Feature home', 'food:Homes navigation:Real Homes'],
            source: 'homes+',
            sortOrder: '11',
            contentTags: ['food:Decorating:Style:Coastal/beach', 'food:Topic:Feature home', 'food:Homes navigation:Real Homes'],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Feature home',
                    urlName: 'feature-home',
                    fullName: 'food_Topic_Feature_home',
                    displayName: 'Feature home'
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                },
                {
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    displayName: 'Coastal/beach'
                }
            ],
            contentMonth: 2,
            contentTitle: 'Empty nesters embrace coastal style',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'Seasoned builder and decorator Suanne Osti has spent the last three years perfecting the decor in her coastal home.',
            pageSearchTags: [
                'Coastal/beach',
                'coastal/beach',
                'Feature home',
                'Feature-home',
                'feature-home',
                'feature home',
                'Real Homes',
                'Real-Homes',
                'real-homes',
                'real homes'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/04/2918/hero_C1487101-4714.jpg',
            pageDateCreated: '2016-02-03T23:23:23.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'coastal style homes'
        },
        {
            id: 'HOMES-2913',
            url: '/6-steps-to-get-your-home-short-term-rental-ready-2913',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2890', 'HOMES-2913'],
            tags: [
                'food:Audience:Renter',
                'food:Topic:Tips and advice',
                'food:Building:Type:Apartment/unit',
                'food:Room:Bedroom:Guest bedroom',
                'food:Topic:How to',
                'food:DIY and craft:How-to:Cleaning/maintainence',
                'food:DIY and craft:DIY projects:Entertaining projects',
                'film:film_genre:Holiday',
                'food:Homes navigation:Bedroom'
            ],
            source: 'homes+',
            sortOrder: '7',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Audience:Renter',
                    urlName: 'renter',
                    fullName: 'food_Audience_Renter',
                    displayName: 'Renter'
                },
                {
                    name: 'food:DIY and craft:DIY projects:Entertaining projects',
                    urlName: 'entertaining-projects',
                    fullName: 'food_DIY_and_craft_DIY_projects_Entertaining_projects',
                    displayName: 'Entertaining projects'
                },
                {
                    name: 'food:DIY and craft:How-to:Cleaning/maintainence',
                    urlName: 'cleaning-maintainence',
                    fullName: 'food_DIY_and_craft_How_to_Cleaningmaintainence',
                    displayName: 'Cleaning/maintainence'
                },
                {
                    name: 'food:Topic:How to',
                    urlName: 'how-to',
                    fullName: 'food_Topic_How_to',
                    displayName: 'How to'
                },
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:Bedroom',
                    urlName: 'bedroom',
                    fullName: 'food_Homes_navigation_Bedroom',
                    displayName: 'Bedroom'
                },
                {
                    name: 'food:Building:Type:Apartment/unit',
                    urlName: 'apartment-unit',
                    fullName: 'food_Building_Type_Apartmentunit',
                    displayName: 'Apartment/unit'
                },
                {
                    name: 'food:Room:Bedroom:Guest bedroom',
                    urlName: 'guest-bedroom',
                    fullName: 'food_Room_Bedroom_Guest_bedroom',
                    displayName: 'Guest bedroom'
                }
            ],
            contentMonth: 2,
            contentTitle: '6 steps to get your home short-term rental ready',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Everything you need to consider before listing your property on a holiday rental site.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/02/03/2913/AirBnB-HERO.jpg',
            pageDateCreated: '2016-02-02T23:47:00.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: '6 steps to get your home short-term rental ready',
            contentImageCaption:
                "Renting out your room or apartment can be a great way to earn a little (or a lot) extra. But before you jump in, make sure you know what you're getting into. Photo: Andrew Finlayson / bauersyndication.com.au"
        },
        {
            id: 'HOMES-2867',
            url: '/homes-february-giveaway-2867',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2792', 'HOMES-2867'],
            tags: ['food:Topic:Decorating 101', 'food:Products, Furniture and Fittings:Accessories and decor', 'food:Homes navigation:DIY'],
            source: 'homes+',
            sortOrder: '34',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Decorating 101',
                    urlName: 'decorating-101',
                    fullName: 'food_Topic_Decorating_101',
                    displayName: 'Decorating 101'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                }
            ],
            contentMonth: 1,
            contentTitle: 'homes+ February 2016 Giveaway',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Enter to win one of these fab prizes!',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/25/2867/Homesplus_Feb_compimage.jpg',
            pageDateCreated: '2016-01-25T00:38:06.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'homes+ February 2016 Giveaway'
        },
        {
            id: 'HOMES-2866',
            url: '/low-cost-makeover-transforms-neglected-rental-2866',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2792', 'HOMES-2866'],
            tags: ['tv:tv_subject:Renovation', 'food:Topic:Before and after', 'food:Homes navigation:Real Homes'],
            source: 'homes+',
            sortOrder: '33',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Before and after',
                    urlName: 'before-and-after',
                    fullName: 'food_Topic_Before_and_after',
                    displayName: 'Before and after'
                },
                {
                    name: 'food:Homes navigation:Real Homes',
                    urlName: 'real-homes',
                    fullName: 'food_Homes_navigation_Real_Homes',
                    displayName: 'Real Homes'
                }
            ],
            contentMonth: 1,
            contentTitle: 'Budget facelift turns eyesore into a standout',
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'With a little TLC, renovation expert Cherie Barber has made a neglected rental home fit for family living.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/25/2866/HEROOprint_dsc_6132a-copy-(2).jpg',
            pageDateCreated: '2016-01-24T23:42:14.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageCaption: 'A few savvy changes have made a dramatic difference to a rundown house in desperate need of an overhaul.'
        },
        {
            id: 'HOMES-2865',
            url: '/cottage-style-garden-with-an-aussie-twist-2865',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2784', 'HOMES-2865'],
            tags: [
                'food:Garden/Outdoor:Landscaping:Backyard/Garden',
                'food:Topic:Gardening 101',
                'food:Garden/Outdoor:Garden style',
                'food:Garden/Outdoor:Plant care:Garden maintenance',
                'food:Garden/Outdoor:Plants:Plants',
                'food:Garden/Outdoor:Plants:Flowers',
                'food:Garden/Outdoor:Landscaping',
                'food:Homes navigation:Outdoor'
            ],
            source: 'homes+',
            sortOrder: '27',
            contentTags: [
                'food:Garden/Outdoor:Landscaping:Backyard/Garden',
                'food:Topic:Gardening 101',
                'food:Garden/Outdoor:Garden style',
                'food:Garden/Outdoor:Plant care:Garden maintenance',
                'food:Garden/Outdoor:Plants:Plants',
                'food:Garden/Outdoor:Plants:Flowers',
                'food:Garden/Outdoor:Landscaping',
                'food:Homes navigation:Outdoor'
            ],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Garden/Outdoor:Plant care:Garden maintenance',
                    urlName: 'garden-maintenance',
                    fullName: 'food_GardenOutdoor_Plant_care_Garden_maintenance',
                    displayName: 'Garden maintenance'
                },
                {
                    name: 'food:Topic:Gardening 101',
                    urlName: 'gardening-101',
                    fullName: 'food_Topic_Gardening_101',
                    displayName: 'Gardening 101'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                },
                {
                    name: 'food:Garden/Outdoor:Plants:Flowers',
                    urlName: 'flowers',
                    fullName: 'food_GardenOutdoor_Plants_Flowers',
                    displayName: 'Flowers'
                },
                {
                    name: 'food:Garden/Outdoor:Plants:Plants',
                    urlName: 'plants',
                    fullName: 'food_GardenOutdoor_Plants_Plants',
                    displayName: 'Plants'
                },
                {
                    name: 'food:Garden/Outdoor:Landscaping:Backyard/Garden',
                    urlName: 'backyard-garden',
                    fullName: 'food_GardenOutdoor_Landscaping_BackyardGarden',
                    displayName: 'Backyard/Garden'
                }
            ],
            contentMonth: 1,
            contentTitle: 'Cottage style garden with an Aussie twist',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'A large acreage in the eastern suburbs of Melbourne has been transformed into a lovely haven of greenery.',
            pageSearchTags: [
                'Backyard/Garden',
                'backyard/garden',
                'Gardening 101',
                'Gardening-101',
                'gardening-101',
                'gardening 101',
                'Garden style',
                'Garden-style',
                'garden-style',
                'garden style',
                'Garden maintenance',
                'Garden-maintenance',
                'garden-maintenance',
                'garden maintenance',
                'Plants',
                'plants',
                'Flowers',
                'flowers',
                'Landscaping',
                'landscaping',
                'Outdoor',
                'outdoor'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/22/2865/Path-HERO.jpg',
            pageDateCreated: '2016-01-22T03:52:19.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery'],
            contentImageAltText: 'Melbourne acreage transformed into haven of greenery'
        },
        {
            id: 'HOMES-1763',
            url: '/save-our-shrubs-how-to-care-for-plants-when-its-hot-1763',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1237', 'HOMES-2791', 'HOMES-2792', 'HOMES-1763'],
            tags: [
                'olympics:olympic_games:Summer',
                'food:Topic:Tips and advice',
                'food:Garden/Outdoor:Plant care:Mulching',
                'food:Garden/Outdoor:Plant care:Weeding',
                'food:Garden/Outdoor:Plant care:Garden maintenance',
                'food:Garden/Outdoor:Garden style:Waterwise garden',
                'food:Homes navigation:Outdoor'
            ],
            source: 'homes+',
            sortOrder: '16',
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Garden/Outdoor:Garden style:Waterwise garden',
                    urlName: 'waterwise-garden',
                    fullName: 'food_GardenOutdoor_Garden_style_Waterwise_garden',
                    displayName: 'Waterwise garden'
                },
                {
                    name: 'food:Garden/Outdoor:Plant care:Weeding',
                    urlName: 'weeding',
                    fullName: 'food_GardenOutdoor_Plant_care_Weeding',
                    displayName: 'Weeding'
                },
                {
                    name: 'food:Garden/Outdoor:Plant care:Garden maintenance',
                    urlName: 'garden-maintenance',
                    fullName: 'food_GardenOutdoor_Plant_care_Garden_maintenance',
                    displayName: 'Garden maintenance'
                },
                {
                    name: 'food:Garden/Outdoor:Plant care:Mulching',
                    urlName: 'mulching',
                    fullName: 'food_GardenOutdoor_Plant_care_Mulching',
                    displayName: 'Mulching'
                },
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:Outdoor',
                    urlName: 'outdoor',
                    fullName: 'food_Homes_navigation_Outdoor',
                    displayName: 'Outdoor'
                }
            ],
            contentMonth: 1,
            contentTitle: "Save our shrubs! How to care for plants when it's hot",
            contentVideo: '',
            nodeTypeAlias: 'HomesArticle',
            contentSummary: 'Garden expert Dale Vine shares his tips on keeping plants healthy in searing heat.',
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2015/08/04/1763/HERO_200811133250826775.jpg',
            pageDateCreated: '2016-01-13T13:00:00.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Articles', 'HomesArticle'],
            contentImageAltText: 'Garden maintenance tools',
            contentImageCaption:
                'Summer gardening demands good timing and regular and wise watering. Photo: Amanda McLauchlan / bauersyndication.com.au'
        },
        {
            id: 'HOMES-2826',
            url: '/brilliant-buys-for-beachy-style-2826',
            path: ['HOMES--1', 'HOMES-1158', 'HOMES-1579', 'HOMES-2783', 'HOMES-2784', 'HOMES-2826'],
            tags: ['food:Topic:Shopping', 'food:Decorating:Style:Coastal/beach', 'food:Topic:Tips and advice', 'food:Homes navigation:DIY'],
            source: 'homes+',
            sortOrder: '15',
            contentTags: ['food:Topic:Shopping', 'food:Decorating:Style:Coastal/beach', 'food:Topic:Tips and advice', 'food:Homes navigation:DIY'],
            contentYear: 2016,
            tagsDetails: [
                {
                    name: 'food:Topic:Shopping',
                    urlName: 'shopping',
                    fullName: 'food_Topic_Shopping',
                    displayName: 'Shopping'
                },
                {
                    name: 'food:Topic:Tips and advice',
                    urlName: 'tips-and-advice',
                    fullName: 'food_Topic_Tips_and_advice',
                    displayName: 'Tips and advice'
                },
                {
                    name: 'food:Homes navigation:DIY',
                    urlName: 'diy',
                    fullName: 'food_Homes_navigation_DIY',
                    displayName: 'DIY'
                },
                {
                    name: 'food:Decorating:Style:Coastal/beach',
                    urlName: 'coastal-beach',
                    fullName: 'food_Decorating_Style_Coastalbeach',
                    displayName: 'Coastal/beach'
                }
            ],
            contentMonth: 1,
            contentTitle: 'Easy add-ons for a chic beach vibe',
            contentVideo: '',
            nodeTypeAlias: 'Gallery',
            contentSummary: 'These cool summer buys will take your coastal-style interiors to new heights.',
            pageSearchTags: [
                'Shopping',
                'shopping',
                'Coastal/beach',
                'coastal/beach',
                'Tips and advice',
                'Tips-and-advice',
                'tips-and-advice',
                'tips and advice',
                'DIY',
                'diy'
            ],
            contentHasVideo: false,
            contentImageUrl: 'https://d3lp4xedbqa8a5.cloudfront.net/s3/digital-cougar-assets/homes/2016/01/25/2826/HERO1201201133639063553-copy.jpg',
            pageDateCreated: '2016-01-13T03:02:05.00Z',
            nodeTypeAliasPath: ['Page', 'Editorial', 'Gallery']
        }
    ]
};
