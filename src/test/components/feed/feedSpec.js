...
describe('Feed Component', () => {
    ...
    describe('feed ads', () => {
        ...
        it('should have an ad as the 3rd item', () => {
            expect(feedListItems[firstAdIndex].getAttribute('class')).to.contain(feedAdClassName);
        });

        it('should render Polar Feed Item at 6th position excluding feed ad', () => {
            expect(feedListItems[polarAdIndex+1].getAttribute('class')).to.contain(polarFeedItemClassName);
        });
        ...
        it(`should have an ad after every ${adSpacing} teasers`, () => {
            for (let i = firstAdIndex; i < feedListItems.length; i++) {
                ...
                if (adExpected) {
                    expect(feedListItems[i].getAttribute('class'), `expected feed item ${i + 1} to be an ad but it was not`)
                        .to.contain(feedAdClassName);
                } else {
                    expect(feedListItems[i].getAttribute('class'), `expected feed item ${i + 1} not to be an ad but it was`)
                        .not.to.contain(feedAdClassName);
                }
            }
        });
